...
With our sample, we use only the cases that include all of the data for these four questions. This is referred to as a __complete case analysis__. That brings our sample of 500 to 439. There are limitations of complete case analyses that we address in the Discussion.   
...
# write_rds(risk2009, "rds/risk2009.rds")
```

```{r}
risk2009 <- read_rds(url("https://github.com/davidkane9/PPBDS/raw/master/rds/risk2009.rds"))

risk2009 %>% skim()
```

Nearly half (48\%) of our sample of 439 youth report that they are trying to lose weight, 54\% percent of the sample are females, and 54\% play on one or more sports teams.
...
10 percent report that they do not watch any TV on school days, whereas another 11\% watched 5 or more hours each day.


```{r}
risk2009 %>%
  summarize(meanBMI = mean(bmipct),
            medianBMI = median(bmipct))
```

The median BMI percentile for our 439 youth is 69.
...
The most dramatic difference in the proportions of those who are trying to lose weight is by sex; 59\% of the females want to lose weight in contrast to only 35\% of the males (see Figure \@ref(fig:mosaicsexlose)). This provides strong support for the inclusion of a sex term in every model considered.

```{r, mosaicsexlose, fig.align="center", out.width="60%", fig.cap='Weight loss plans vs. Sex', warning=FALSE, comment=NA}
risk2009 %>%
  ggplot(aes(x = lose.wt, y = ..prop.., group = sex, fill = sex)) +
...
risk2009 %>%
  ...
  summarize(mean_BMI = mean(bmipct),
            sd_BMI = sd(bmipct),
            n = n()) %>%
  kable(col.names = c("Sex", "Lose weight", "Mean BMI", "SD BMI", "Sample size"),
        caption = "Mean BMI percentile by sex and desire to lose weight",
        digits = 0)
```
...
```{r, logitBMIsex, fig.align="center",out.width="60%", fig.cap='Empirical logits of trying to lose weight by BMI and Sex.', warning=FALSE, comment=NA}
risk2009 %>%
  group_by(sex) %>%
  ...
  mutate(prop.lose = ifelse(prop.lose == 0, .01, prop.lose),
         emplogit = logit(prop.lose)) %>%
  ggplot(aes(x = midpoint, y = emplogit, color = sex)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
```{r, mosaicsexsports, fig.align="center", out.width="60%", fig.cap='Weight loss plans vs. sex and sports participation', warning=FALSE, comment=NA}
risk2009 %>%
  ggplot(aes(x = lose.wt, y = ..prop.., group = sex, fill = sex)) +
  geom_bar(position = "dodge") +
  labs(x = "Lose weight",
...
```{r, logitmediasex, fig.align="center",out.width="60%", fig.cap='Empirical logits for the odds of trying to lose weight by TV watching and sex.', warning=FALSE}
risk2009 %>%
  group_by(sex, media) %>%
  summarise(prop.lose = mean(lose.wt.01), n = n()) %>%
  mutate(prop.lose = ifelse(prop.lose == 0, .01, prop.lose)) %>%
  mutate(emplogit = logit(prop.lose)) %>%
  ggplot(aes(x = media, y = emplogit, color = sex)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
Our strategy for modeling is to use our questions of interest and what we have learned in the exploratory data analysis. For each model we interpret the coefficient of interest.

We first use a model where sex is our only predictor.

```{r, 6model1, comment=NA}
risk2009 <- risk2009 %>%
...
\[\log\left(\frac{{p}}{1-{p}}\right)=-0.60+0.94 (\textrm{female})\]
where ${p}$ is the estimated proportion of youth wanting to lose weight. We can interpret the coefficient on female by exponentiating $e^{0.94} = 2.57$ (95% CI = $(1.75, 3.80)$) indicating that the odds of a female trying to lose weight is over two times the odds of a male trying to lose weight. We retain sex in the model and consider adding the BMI percentile:



```{r, 6model2int, include=FALSE, comment=NA}
...
We see that BMI is positively associated with the odds of trying to lose weight, after controlling for sex.  Clearly BMI percentile belongs in the model with sex.  

Our estimated logistic regression model is:
\[\log\left(\frac{{p}}{1-{p}}\right)= -3.40+1.36\textrm{female}+0.039\textrm{bmipct}\]

To interpret the coefficient on `bmipct`, we will consider a 10 unit increase in `bmipct`.  Because $e^{10*0.039}=1.477$, then there is an estimated 47.7\% increase in the odds of wanting to lose weight for each additional 10 percentile points of BMI for members of the same sex. Just as we had done in other multiple regression models we need to interpret our coefficient *given that the other variables remain constant*. An interaction term for BMI by sex was tested as well, but appears that the effect of BMI differs by sex.
...
```{r, 6model3, comment=NA, message=FALSE}
...
Sports teams do not appear to matter in any of these models, nor do interaction terms (sex by sports) and (bmipct by sports). As a result, sports participation was no longer considered for inclusion in the model.

We last look at adding `media` to our model.

```{r, 6model4, comment=NA, message=FALSE}
...
Media is a small and imprecisely estimated term. We have no support in our data that media exposure as measured by hours spent watching TV is associated with the odds a teen is trying to lose weight after accounting for sex and BMI.
...
Grabe, Shelly, Janet Shibley Hyde, and L. Moniquee Ward. 2008. “The Role of the Media in Body Image Concerns Among Women: A Meta-Analysis of Experimental and Correlational Studies.” Pyschological Bulletin 134 (3): 460–76. doi:10.1037/0033-2909.134.3.460.

Martinsen, M, S Bratland-Sanda, A K Eriksson, and J Sundgot-Borgen. 2009. “Dieting to Win or to Be Thin? A Study of Dieting and Disordered Eating Among Adolescent Elite Athletes and Non-Athlete Controls.” British Journal of Sports Medicine 44 (1): 70–76. doi:10.1136/bjsm.2009.068668.
