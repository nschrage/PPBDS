...
house_region_tree <- fit(tree_fit,
...
```{r}
house_year_tree_0 <- fit(update(tree_fit, cost_complexity = 0),
                         factor(dem_win) ~ year,
                         data = house_ch13)
...
```{r, message = FALSE}
tree_fit_tune <- decision_tree(cost_complexity = tune()) %>%
  set_engine("rpart",
             model = TRUE) %>%
  set_mode("classification")
...
set.seed(1234)
house_boots <- bootstraps(house_ch13, times = 25)

cost_grid <- grid_regular(cost_complexity(range = c(0, 0.1), trans = NULL), levels = 21)
...
# this is slow so what if we did it this way?

# house_boots <- house_ch13 %>%
#   rep_sample_n(size = nrow(house_ch13), replace = TRUE, reps = 25)
# 
# house_boots %>%
#   group_by(replicate) %>%
#   nest() %>%
#   mutate(house_year_tree = map(data, ~ fit(update(tree_fit, cost_complexity = 0),
#                                            factor(dem_win) ~ year,
#                                            data = .)))
# 
# tune_model <- function(boots, model, tune_param, formula) {
#   boots %>%
#     group_by(replicate) %>%
#     nest() %>%
#     mutate(mod = map(data, ~ fit(update(model, tune_paramtuning, formula, data = .))
# }

best_cost <- select_best(tree_res, metric = "accuracy")

tree_fit_best <- decision_tree(cost_complexity = best_cost) %>%
  set_engine("rpart",
             model = TRUE) %>%
  set_mode("classification")

house_year_best <- fit(tree_fit_best,
                       factor(dem_win) ~ year,
                       data = house_ch13)
```
