...
## Presidential elections: logistic regression
...
Figure \@ref(fig:OLSlogistic) illustrates a data set with a binary (0 or 1) response ($Y$) and a single continuous predictor ($X$).  The blue line is a linear regression to model the probability of a success ($Y=1$) for a given value of $X$.  With a binary response, the line doesn't fit the data well, and it produces predicted probabilities below 0 and above 1.  On the other hand, the logistic regression fit (red curve) with its typical "S" shape follows the data closely and always produces predicted probabilities between 0 and 1.  Here is the formula for a logistic regression:

\begin{equation}
log(\frac{p_i}{1-p_i})=\beta_0+\beta_1 x_i
(\#eq:logisticReg)
\end{equation}
where the observed values $Y_i \sim$ Bernoulli with $p=p_i$ for a given $x_i$ and $n=1$.

### Exploratory data analysis

The data on presidential results by state from 1976 to 2016 can be found in the `pres_results_by_cd` **politicaldata** package.  We'll create a version of this data frame called `pres_ch13` that creates a new column `dem_win` that notes for each state if the Democratic candidate received more votes than the other candidates.  We'll also join it with the `state_info` data frame in the **fivethirtyeight** package to add the `region` of each state.

```{r}
library(politicaldata)

pres_ch13 <- pres_results %>%
  ...
  select(region, state, year, dem_win)
```
...
Let's first look at the raw data values by either looking at `pres_ch13` using RStudio's spreadsheet viewer or by using the `glimpse()` function from the **dplyr** package:

```{r}
glimpse(pres_ch13)
```

Let's also display a random sample of 5 rows of the 4,201 rows corresponding to different district-years. Remember due to the random nature of the sampling, you will likely end up with a different subset of 5 rows.
...
(\#eq:binreg)
 \end{equation}
Models of this form are referred to as __logistic regression models__. \index{logistic regression models}  Here we provide intuition for using and interpreting logistic regression models.

\begin{equation}
\log\left(\frac{p_X}{1-p_X}\right)=\beta_0 +\beta_1X
(\#eq:logitXform)
\end{equation}
where $p_X$ is the probability of success given $X$.  

Let's say that $X$ is a single binary variable. Based on this model, the log odds of a successful penalty kick when $X = 0$ is:
\[
\log\left(\frac{p_0}{1-p_0}\right) =\beta_0 \nonumber,
\]
and the log odds when $X = 1$ is:
\[
\log\left(\frac{p_1}{1-p_1}\right)=\beta_0+\beta_1. \nonumber
\]

We can see that $\beta_1$ is the difference between the log odds of success when $X = 1$ versus $X = 0$.  Using rules of logs:
\begin{equation}
