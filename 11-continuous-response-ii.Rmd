...
<!-- EC: Potential tutorial questions (to put into Gov-50/part_3 once that's forkable): -->
<!-- **Interaction Effects** -->
<!-- * Calculate for evaluation scores, regressing on age and gender and taking their interaction effects. -->
<!-- * What is the "offset" for a 60-year-old male instructor? -->
<!-- * How would you interpret the interaction effect between age and gender for male instructors? -->
...
score_model_interaction <- lm(score ~ age * gender, data = earnings)
...
```{r}
lm(score ~ I(age - 30) * gender, data = earnings) %>%
  tidy(conf.int = TRUE) %>%
...
gendermale_coefs <- tibble(ages = 20:90) %>%
  ...
  mutate(mod = map(ages, ~ lm(score ~ I(age - .) * gender, data = earnings)),
...
```{r eval=FALSE}
ggplot(earnings, aes(x = age, y = score, color = gender)) +
  geom_point() +
...
par_slopes <- ggplot(earnings, aes(x = age, y = score, color = gender)) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", color = "Gender") +
  geom_parallel_slopes(se = FALSE)
if(knitr::is_html_output()){
...
score_model_parallel_slopes <- lm(score ~ age + gender, data = earnings)
...
interaction_plot <- ggplot(earnings, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
  theme(legend.position = "none")
parallel_slopes_plot <- ggplot(earnings, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_parallel_slopes(se = FALSE) +
...
fitted_plot <- ggplot(earnings, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
