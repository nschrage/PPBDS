...
<!-- EC: There are 2 moderndive datasets and the parallel slopes function from the `moderndive` package that are used extensively in this chapter. Does the `moderndive` removal affect just datasets, or functions as well? -->

<!-- EC: Dataset replacement proposals
UT Austin example: Qscores. I need another datset regressing on one numerical and one categorical variable, also the datasets are similar in that they analyze evaluation scores. 
Credit card debt example: Use a dataset with one in which there is a numerical outcome variable and 2 numerical explanatory variables, don't know which one is most fitting
...
## Teaching evaluations revisited: one numerical and one categorical explanatory variable {#model4}

<!-- EC: Check which dataset was used before in introducing regression on a numerical/categorical explanator variable? Presumably need to replace this dataset.-->

Let's revisit the instructor evaluation data from UT Austin we analyzed previously. We studied the relationship between teaching evaluation scores as given by students and "beauty" scores. The variable teaching `score` was the numerical outcome variable $y$, and the variable "beauty" score (`bty_avg`) was the numerical explanatory $x$ variable. 
...
Recall that data on the 463 courses at UT Austin can be found in the `evals` data frame included in the **moderndive** package. However, to keep things simple, let's `select()` only the subset of the variables we'll consider in this chapter, and save this data in a new data frame called `evals_ch11`. Note that these are different from the variables previously chosen. 
...
Let's also display a random sample of 5 rows of the 463 rows corresponding to different courses in Table \@ref(tab:model4-data-preview).
...
Due to the random nature of the sampling, you will likely end up with a different subset of 5 rows. The `set.seed()` function allows you to "randomly" obtain the same 5 rows if you set the same number as the seed. In this case, we've set the seed to 1, arbitrarily. 
...
evals_ch11 %>%
  ...
  gt() %>%
   tab_header(
    title = "A random sample of 5 out of the 463 courses at UT Austin")
```

Now that we've looked at the raw values in our `evals_ch11` data frame and got a sense of the data, let's compute summary statistics. As done before, let's use the `skim()` function from the **skimr** package, being sure to only `select()` the variables of interest in our model.
...
Furthermore, let's compute the correlation coefficient between our two numerical variables: `score` and `age`. Recall that correlation coefficients only exist between numerical variables. We observe that they are "weakly negatively" correlated.
...
Let's now create a visualization to display the relationship between the outcome variable `score` and the explanatory variable `age`. Since both variables are numerical, we'll use a scatterplot to display their relationship. We can incorporate the categorical variable `gender` by adding `color = gender` to the `aes()`thetic mapping.
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Colored scatterplot of relationship of teaching and beauty scores.", fig.height=3.2}
if(knitr::is_html_output()){
  ggplot(evals_ch11, aes(x = age, y = score, color = gender)) +
    geom_point() +
    labs(x = "Age", y = "Teaching Score", color = "Gender") +
    geom_smooth(method = "lm", se = FALSE)
} else {
...
In the resulting Figure \@ref(fig:numxcatxplot1), observe that the `geom_smooth(method = "lm", se = FALSE)` layer automatically fits a different regression line for each group. 
...
Going back to our multiple regression model for teaching `score` using `age` and `gender` in Figure \@ref(fig:numxcatxplot1), we generate the regression table using a two-step approach: we first "fit" the model using the `lm()` "linear model" function and then we apply the `tidy()` function. This time, however, our model formula won't be of the form `y ~ x`, but rather of the form `y ~ x1 * x2`. In other words, our two explanatory variables `x1` and `x2` are separated by a `*` sign:

```{r, eval=FALSE}
# Fit regression model:

score_model_interaction <- lm(score ~ age * gender, data = evals_ch11)
...
There are four rows of values corresponding to a different predictor. Female instructors are the baseline group for comparison, as the other predictors display the additional "offset" amount for males. Thus, `intercept` is the intercept *for only the female instructors*. 
...
The value for `gendermale` is the *offset* in intercept for male instructors relative to female instructors. The intercept for the male instructors is `intercept + gendermale` = 4.883 + (-0.446) = 4.883 - 0.446 = 4.437. Similarly, `age:gendermale` = 0.014 is not the slope for age for the male instructors, but rather the *offset* in slope for the male instructors. Therefore, the slope for age for the male instructors is `age + age:gendermale` $=  -0.018 + 0.014 = -0.004$.
...
Since the slope for age for the female instructors was -0.018, it means that on average, a female instructor who is a year older would have a teaching score that is 0.018 units **lower**, whereas the corresponding associated decrease for males was on average only 0.004 units. This is consistent with our observation from Figure \@ref(fig:numxcatxplot1), that this model is suggesting that age impacts teaching scores for female instructors more than for male instructors, as the slope for age for females is more negative than that of males.
...
<!-- EC: Removed infer and replaced with rsample::bootstraps -->
...
x <- evals_ch11 %>%
  ...
  mutate(mod = map(splits, ~ lm(score ~ age * gender, data = .)),
...
```{r}
lm(score ~ I(age - 30) * gender, data = evals_ch11) %>%
  tidy(conf.int = TRUE) %>%
...
gendermale_coefs <- tibble(ages = 20:90) %>%
  ...
  mutate(mod = map(ages, ~ lm(score ~ I(age - .) * gender, data = evals_ch11)),
...
gendermale_coefs %>%
  ...
  theme_minimal() +
  labs(x = "Age",
       y = "Effect of being male on teaching scores",
       title = "Predicting teaching scores with gender and age",
       subtitle = "As age increases, being male is more associated with higher teaching scores")
...
Another model we can use to visualize regressing on one numerical variable and one categorical variable is known as a *parallel slopes* model. Unlike interaction models where the regression lines can have different intercepts and different slopes, parallel slopes models still allow for different intercepts but *force* all lines to have the same slope. This is done by using the indicator variable we previously introduced, ${1}_{\text{is male}}$. Each line will correspond to an outcome of this indicator variable. Let's visualize the best-fitting parallel slopes model to `evals_ch11`.

<!-- EC: Remove moderndive package??-->

A function called `geom_parallel_slopes()` in the **moderndive** package allows us to plot the parallel slopes model for teaching score. Notice how the code is identical to the code that produced the visualization of the interaction model previously, but now the `geom_smooth(method = "lm", se = FALSE)` layer is replaced with `geom_parallel_slopes(se = FALSE)`. 
...
```{r echo=FALSE, warning=FALSE, fig.cap="Parallel slopes model of score with age and gender.", fig.height=3.5}
par_slopes <- ggplot(evals_ch11, aes(x = age, y = score, color = gender)) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", color = "Gender") +
  geom_parallel_slopes(se = FALSE)
if(knitr::is_html_output()){
...
score_model_parallel_slopes <- lm(score ~ age + gender, data = evals_ch11)
...
interaction_plot <- ggplot(evals_ch11, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
  theme(legend.position = "none")
parallel_slopes_plot <- ggplot(evals_ch11, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_parallel_slopes(se = FALSE) +
...
newpoints <- evals_ch11 %>% 
  slice(c(1, 5)) %>% 
  augment(score_model_interaction, newdata = .)

fitted_plot <- ggplot(evals_ch11, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
...
Now what if we want the fitted values not just for these two instructors, but for the instructors of all 463 courses included in the `evals_ch11` data frame? Doing this by hand would be long and tedious! This is where the `augment()` function from the **broom** package can help: it will quickly automate the above calculations for all 463 courses. We present a preview of just the first 10 rows out of 463 in Table \@ref(tab:model4-points-table).  Just like in the last chapter, we'll use the `.se.fit` column to construct confidence intervals around our predicted values.

```{r, eval=FALSE}
regression_points <- score_model_interaction %>%
  augment() %>%
  mutate(conf.low = .fitted - 2 * .se.fit,
         conf.high = .fitted + 2 * .se.fit) %>%
  select(score, age, gender, .fitted, conf.low, conf.high, .resid)
regression_points
```
```{r echo=FALSE}
regression_points <- augment(score_model_interaction) %>%
  mutate(conf.low = .fitted - 2 * .se.fit,
         conf.high = .fitted + 2 * .se.fit) %>%
  select(score, age, gender, .fitted, conf.low, conf.high, .resid)
regression_points %>%
...
Let's load the `Credit` dataset. To keep things simple let's `select()` the subset of the variables we'll consider in this chapter, and save this data in the new data frame `credit_ch12`. Notice our slightly different use of the `select()` verb here than we introduced in Subsection \@ref(select). For example, we'll select the `Balance` variable from `Credit` but then save it with a new variable name `debt`. We do this because here the term "debt" is easier to interpret than "balance."
...
Furthermore, let's look at a random sample of five out of the `r nrow(credit_ch12)` credit card holders in Table \@ref(tab:model3-data-preview). Once again, note that due to the random nature of the sampling, you will likely end up with a different subset of five rows.
...
Note that unlike our example with teaching scores and gender, we don't have heterogeneous treatment effects, so we are applying the same estimate of the ATE to all three people. To save space, we won't display the ATE ($-7.663$ $(-8.420,$ $-6.906)$) in the table below.
