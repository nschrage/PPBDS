...
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point()
```
We have already learned about different ways to make this plot more comprehensible. We can make the points transparent with the alpha argument, or we can use geom_jitter() to shake the points. A rather rarely used but sometimes helpful alternative is geom_bin2d(), which creates a so-called *heatmap*:
...
Notice that you can choose any other variable for the z-value that is not directly related to the other two. For example, you could have chosen the amount of water ejected by the Geyser to include a third measure. But since this is rather difficult to interpret, it often makes sense to just take the density of the two-dimensional distribution as z-value we did like above. 
...
- `labs()` adds a title, subtitle, caption, label on the x axis, and label on the y axis. You should use them in every plot. If you choose not to --- and that is often a reasonable choice --- you should explain why in a code comment.
...
Observe in both Figures \@ref(fig:facethistogram) and \@ref(fig:facethistogram2) that as we might expect in the Northern Hemisphere, temperatures tend to be higher in the summer months, while they tend to be lower in the winter. See the ggplot2 book for [more details](https://ggplot2-book.org/facet.html).


### Other Techniques

<!-- DK: Add an example plot which uses example tricks which are most important from these. -->

See the latest edition of the ggplot2 book for information about [statistical summaries](https://ggplot2-book.org/statistical-summaries.html),  [legends and axes](https://ggplot2-book.org/guides.html),  [scales](https://ggplot2-book.org/scales.html), [coordinate systems](https://ggplot2-book.org/coord.html), [themes](https://ggplot2-book.org/polishing.html). You can set the axes limits with `xlim()` and `ylim()`.
