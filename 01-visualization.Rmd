...
<!-- - n(), which counts total # of rows [in each group], especially alongside group_by() and summarize(). -->
<!-- - coord_cartesian() and xlim/ylim, which allows for zooming/rescaling graphs. -->
...
<!-- In addition, some functions that I don't use super often but would be nice to include in the textbook if you want to: -->
<!-- - the max() and min() functions, especially alongside summarize(). -->
<!-- - dplyr's ntile() function to turn continuous data into categorical data. -->
<!-- - the n_unique() function, a more selective version of n(). -->
...
These packages contain datasets which we will use when creating visualizations. You may need to install these packages if you have not already.
...
<!-- Maybe add book cover for wilkinson2005 -->

```{marginfigure}
**ggplot2** is rooted in the data visualization theory known as _the grammar of graphics_ developed in @wilkinson2005. Similarly to a linguistic grammar, "the grammar of graphics" defines a set of rules for constructing *statistical graphics* by combining different types of *layers*. 
```
...
```{r, echo=FALSE, fig.cap="Artwork by Allison Horst", fig.margin = TRUE}
...
As an alternative to `geom_boxplot()`, we can also try `geom_violin()` which is called a violin plot. A violin plot is a compact display of a continuous distribution which blends `geom_boxplot()` and `geom_density()` (which you'll learn about soon). Therefore, a violin plot is a mirrored density plot displayed in the same way as a boxplot, where the mirrored density distribution in `geom_violin()` replaces the box and whiskers in `geom_boxplot()`.

Let's use the last plot we created in the `geom_boxplot()` section to explore `geom_violin()` more.

```{r}
ggplot(data = weather, mapping = aes(x = factor(month), y = temp)) +
  geom_violin()
```
The resulting plot shows 12 separate "mirrored density" plots shown "side-by-side."

You could also combine `geom_violin()` and `geom_boxplot()` together to add summary statistics to the violin plot.
```{r}
ggplot(data = weather, mapping = aes(x = factor(month), y = temp)) +
...
One complication in creating barplots is determining how your data is represented. Is the categorical variable of interest "pre-counted" or not? 

Let's create another data frame that also represents a collection of 3 apples and 2 oranges called `fruits_counted`.
...
We see both the `fruits`, used in the `geom_bar()` section, and `fruits_counted` represent the same collection of fruit. Whereas `fruits` just lists the fruit individually...
...
Let's generate barplots using these two different representations of the same basket of fruit: 3 apples and 2 oranges. Here's the plot we created in the previous section using `geom_bar()`:
...
Using the `fruits_counted` data frame where the fruits have been "pre-counted", we once again map the `fruit` variable to the x-position aesthetic, but here we also map the `number` variable to the y-position aesthetic, and add a `geom_col()` layer instead.
...
Let's again look at the `flights` data frame in the `nycflights13` package and visualize the distribution of the categorical variable `carrier`. Alternatively, say you had a data frame where the number of flights for each `carrier` was pre-counted.
...
```{r fig.cap='Number of flights departing NYC in 2013 by airline using `geom_col()`.'}
...
This geom can be a useful alternative to geom_point() in case of overplotting. Remember the plot at the beginning where we looked at the relationship between arrival delay and departure delay of flights from Alaska Airlines:

```{r fig.align="center", warning=TRUE, echo=TRUE}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point()
```

We have already learned about different ways to make this plot more comprehensible. It is possible to make the points transparent with the alpha argument, or to shake them by using geom_jitter(). A rather rarely used but sometimes helpful alternative is geom_bin2d(), which creates a so-called *heatmap*:
...
The advantage of heatmaps over the other variants is that we get a legend with a concrete value for the number of observations. However, compared to the scatterplot above, we have less information about where exactly the single observations fall. 

### `geom_density2d()`

You may have noticed that the plot we just saw above is actually just a two-dimensional bar plot (hence the name). Similarly, it is possible to create two-dimensional density plots using geom_density2d(). Let's see how this looks like using the data from before (we excluded some values for better visibility):

```{r fig.align="center", warning=TRUE, echo=TRUE, eval=FALSE}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point() +
  geom_density2d()
```
...
You can see that R draws a lot of circles. These are drawn to separate the data into equal shares, e.g. there are as many points inside the smallest circle as outside the largest. 

While it would have been possible to use only geom_density2d(), we would then only see the circles without knowing how many values they contain. Therefore, geom_density2d() is used as an addition rather than an alternative to geom_point(). The biggest advantage of a two-dimensional density plot is that no artificial border is drawn, which was the case before when we used geom_bin2d(). 

### `geom_contour_filled()`

While the ggplot2 package can not draw actual 3D surfaces, we can geom_contour_filled() visualize 3D surfaces in 2D. To do this, three different variables must be specified. The following plot uses the *faithfuld* dataset of ggplot2, which contains data about the Old Faithful Geyser in Wyoming:
...
The rings here again have the same interpretation as in the two-dimensional density plot. The colors represent the z-axis and plot a variable that measures the density of the two-dimensional distribution. In a real 3D graph, the violet area would be flat, and the bright areas would form two hills in the graph - the brighter the higher (and the denser the distribution of values).

Notice that you can also choose any other variable for the z-value that is not directly related to the other two. For example, you could have chosen the amount of water ejected by the Geyser to include a third measure. But since this is rather difficult to interpret, it often makes sense to just take the density of the two-dimensional distribution as z-value we did like above. 


## Advanced Plots

```{r, echo=FALSE, fig.cap="Data Visualization with ggplot2 Cheat Sheet"}
knitr::include_graphics("01-visualization/images/ggplot2-cheatsheet.jpg")
```

In the previous section we have seen the three components that every plot must include: data, data mappings, and a geom. You may have found that the graphics from before look a bit boring. Luckily, the Grammar of Graphics allows us to add more layers to our own to customize our plots. We will go over some of the additional layers here, but the [Data Visualization with ggplot2 Cheat Sheet](https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf) is a great resource to refer to as your ggplot visualizations get more complicated.

<!-- Links to relevant readings in Healy.  -->

To keep it simple, we will change the Gapminder plot from the beginning of the chapter layer by layer:
...
```{r fig.align="center", warning=TRUE, echo=TRUE }
ggplot(data = gapminder_07, mapping = aes(x = gdpPercap, y = lifeExp, color = continent)) +
  geom_point()
```

At the moment it consists of only three necessary elements: 

* The gapminder dataset, filtered by the year 2007 (data)
* GDP per capita on the x-axis, and life expectancy on the y-axis (mappings)
* geom_point(), which creates a scatterplot (geom)
...
This is way better! However, R chooses by default 3 plots per row, so that Europe and Oceania are below the other three continents. We can specify the number of rows and columns in the grid by using the `nrow` and `ncol` arguments inside of `facet_wrap()`. Let's get all continents in a row by setting nrow = 1:
...
Now we not only recognize the differences between the continents, but also within the continents better. However, the interpretation of box plots is not so intuitive, at least at first glance. An easier to understand alternative is a density plot, which we can add with `stat_ydensity()`:

```{r fig.align="center", message=FALSE, warning=FALSE}
ggplot(data = gapminder_07, mapping = aes(x = gdpPercap, y = lifeExp, color = continent)) +
  geom_point() +
  facet_wrap(~ continent, nrow = 1) +
  stat_ydensity(aes(alpha = 0.5))
```
Notice that we used the alpha argument again here. This is optional, but it makes the plot clearer. You can also see that R did not draw a line for Oceania - this is normal and often happens when there is not enough data for an operation.
...
Besides the position we can also manipulate the limits of the axes by using `xlim()` and `ylim()`. For example, assume that we are only interested in countries with a GDP per capita from 0 to 30000. We can tell R as follows that we only want to see this range:
...
We can see that the GDP per capita on the y-axis is now only shown from 0 to 30000. But why did we use `xlim()` when we changed the y-axis? The reason is that `xlim()` does not refer to the axes, but to the mapping. Remember that the first line of the code is *x = gdpPercap*. Although we used `coord_flip()` to flip the axes, this only affects the visual representation and not the variable specification. Don't worry if that doesn't seem intuitive. You will quickly notice once you set the axis limits the wrong way round (try to run the same code using `ylim()`).

Finally we can change the scaling of the axes. For example, it might be useful to display the axes with `scale_x_log10()` or `scale_y_log10()` on a logarithmic scale. Let's try this out on GDP per capita:
...
That looks good! Notice how the scale of GDP per capita has changed. Just like above, we referred to the x-variable again, although gdpPercap is on the y-axis. It should be noted here that this "problem" only occurs with axis limits/scales, and only if we switched axes before (which happens rarely). The point here is just to understand how data mappings work in gglplot.
...
It is up to us, which or how many of the arguments in `labs()` we use. While most arguments are probably self-explanatory, it makes sense to have a look at the last two. These determine the title of the legends, and should always be named after the aesthetic to which the legend refers. Since the first legend was created by the *alpha* argument in the density plot, we can refer to it with "alpha =". The same applies to the second legend, which was created by the *color* argument.
...
ggplot(data = gapminder_07, mapping = aes(x = gdpPercap, y = lifeExp, color = continent)) +
  ...
  labs(
    ...
    color = "Continent",
    alpha = "Transparency") +
  geom_text(aes(label = country), size = 2, color = "black", check_overlap = TRUE)
```

Great! Now we also see that the two lonely countries in Oceania are Australia and New Zealand. Notice that we need to determine an aestethic called *label*. This defines the character variable which will be used as the basis for the labels.
...
In addition to ready-to-use themes, the `theme()` function also offers a wide [selection](https://ggplot2.tidyverse.org/reference/theme.html) of functions for manually changing individual elements. Let's remove the two legends, change the font in the plot and enlarge the axis labels a bit:
...
<!-- DK: Something here. Does not need to be long, but shows ifelse() used with mututate. Are there other functions which we should mention here? -->
...
Congratulations for making it to the end of the chapter. At this point you have already learned many different concepts and methods to visualize and manipulate data. In the first section we looked at basic concepts and terms we are dealing with when programming with R. In section two, we learned about the three basic components that make up each plot: Data, mapping, and one or multiple geoms. The ggplot package offers a wide range of geoms that we can use to create different types of plots. In the third section we looked at some additional elements that we can use to modify our plots. These include features such as axis scaling, labeling and themes, and although we can also create plots without them, they often determine how much information we can derive from our data. Finally, in the fourth section, we took a look at the "super package" tidyverse, which also includes ggplot. Besides tools for visualization, it offers features for importing and manipulating data, which is the main topic of the following chapters.
...
# TW: Is there a way to attach ploty so it can simply be loaded like tidyverse?
...
knitr::include_graphics("01-visualization/images/rayshade-plot.png")
```
