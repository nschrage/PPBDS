...
<!-- 2) Purpose of Advanced plotting is to show the tools for making the plots that appear in Healy. -->
...
We'll begin by exploring the `trains` data frame in the `PPBDS.data` package and get an idea of its structure. This dataset includes data for attitudes toward immigration-related policies, both before and after an experiment which randomly exposed a treated group to Spanish-speakers on a Boston commuter train platform. Individuals with a treatment value of "Treated" were exposed to two Spanish-speakers on their regular commute. "Control" individuals were not.
...
There is a subtle difference between the kinds of variables that you will encounter in data frames. There are *identification variables* and *measurement variables*. For example, let's explore the `qscores` data frame also in the `PPBDS.data` package by showing the output of `glimpse(qscores)`:
...
### Components of a graphic and the grammar of graphics
...
```{r echo=FALSE}
tibble(`data variable` = c("GDP per Capita", "Life Expectancy", 
...
DK: Compare and contrast with geom_histogram. Perhaps even 
