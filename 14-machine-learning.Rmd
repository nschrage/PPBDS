...
```{r}
cces <- cces %>%
  select(pres_approval, pres_gop, pid7, ideo5, race, female, educ, age, state) %>%
...
1. `pid7` and `ideo5` plus demographic variables (`race`, `female`, `educ`, and `age`)
1. Same as above, but with interactions between `race` and `educ` and between `female` and `age`
...
As we'd expect, Republicans and conservatives approve of Republican presidents more.
...
Here we see that the basic model and the state model perform worse and the other three models, all of which include demographic covariates, perform better.
...
cces_folds <- vfold_cv(cces_train, v = 5)
```
...
It's not that hard to extend this to all of our formulas using `map()`.  (Note that `fit_resamples()` currently gives the warning message "prediction from a rank-deficient fit may be misleading" whenever a factor or character variable is used as a predictor; this does not necessarily mean that the fit was actually rank-deficient.)

```{r, message = FALSE}
cces_metrics <- cces_formulas %>%
...
Let's present the results stored in our `cces_metrics` object:
