...
3) Let's now turn our attention towards the `qscores` data from the `PPBDS.data` package. This data set contains course evaluation data on all courses taught at Harvard in the last two years. The data to be used has already been prepared for you. View it below by running `qscores`. 

```{r, echo = FALSE}

qscores1 <- PPBDS.data::qscores%>% 
  select(course_name, department, workload, overall) %>% 
  sample_n(100) %>% 
  group_by(department) %>% 
  nest() 
...
```{r, C0, echo=TRUE, exercise = TRUE}

qscores1

```

Note that the data has been grouped by department, with all other information nested together, much like in the form of a list column. We will now use `map_dbl()` to calculate the `mean()` of each of the department's overall scores. We do this by wrapping `(data$overall)` in the `mean()` function. 

In the following exercise, create a new column called `mean_overall` for this purpose. 

```{r, C1, exercise = TRUE, eval = TRUE, echo = TRUE}
...
```{r C1-hint-1}
"Wrap `(.$overall)` in the `mean()` function to create the `mean_overall` column variable."
```
...
qscores1 %>% 
  mutate(mean_overall = map_dbl(data, ~ mean(.$overall))) 
...
This exercise allowed us to use a `map_*()` function to get access to a particular column of the data frames stored in the data list, after it had been nested. Additionally, it allowed us to perform operations on that column, and use it to create a new one altogether - one that successfully computed the mean of `overall` scores for each departments' courses. 

4) I now want to round the overall mean scores to the nearest whole number. Once more, create a column called `rounded_means` that utilizes the `map_dbl()` function in conjunction with the `round()` function for this particular purpose. The data has been prepared for you. 
...
```{r, C2, exercise = TRUE, eval = TRUE, echo = TRUE}

# Round the values of the overall means to the nearest whole
# number. 

qscores1 %>% 
  mutate(rounded_means = ----------------------)

```

```{r C2-hint-1}
"Wrap the `round` function in the `map_dbl()` function to create the `rounded_means` column."
```

```{r, C2-solution}

qscores1 %>% 
  mutate(rounded_means = map_dbl(mean_overall, round)) 
...
5) Let's now turn towards using `map_lgl()`. We want to use `map_lgl()` to create a condition that reports `TRUE` if the `mean_overall` score for each department is greater than 4. We will name this logical column `gthan4`. 

Use the condition `mean_overall > 4` within a `map_lgl()` function to compute the output column `gthan4`. 
...
```{r, C3, exercise = TRUE, eval = TRUE, echo = TRUE}
...
```{r C3-hint-1}
"Use `~ mean_overall > 4` to create the relevant condition. Note that you're using an anonymous function for this process!"
```

```{r, C3-solution}

qscores1 %>% 
  mutate(gthan4 = map_lgl(mean_overall, ~ mean_overall > 4)) 
...
Notice that you could have also used `~ . mean_overall > 4` for the same outcome. Try it in the section above, and run your answer to see the results you obtain!

6) Now, lets replace `map_lgl()` with `map_dbl()`, maintaining the same condition. What do we expect to see? 

```{r, setup3, echo = FALSE, eval = FALSE}

qscores1

```

```{r, C4, exercise = TRUE, eval = TRUE, echo = TRUE}

qscores1 %>% 
  mutate(gthan4 = -----(mean_overall, ~ mean_overall > 4)) 
...
qscores1 %>% 
  mutate(gthan4 = map_dbl(mean_overall, ~ mean_overall > 4)) 
...
grade_code("Correct! Instead of giving you a response in `TRUE` and `FALSE`, we now compute logical values in the form of 1s and 0s, given that we're still mapping results using a condition.")

```

7) What if we wanted to use `map_chr()` to use the same condition, but give us different character outputs? Let's say we want the `gthan4` column to report `I do not like this dept.` for every department  that has an overall mean q score rating of below 4, and `I love this dept` for every department with an overall mean q score rating of above 4. 

We will employ `if_else()` for this purpose.


```{r, setup4, echo = FALSE, eval = FALSE}

qscores1 

```

```{r, C5, exercise = TRUE, eval = TRUE, echo = TRUE}

qscores1 %>% 
  mutate(gthan4 = -----(mean_overall, ~ --------------------------------------------)) 

```

```{r C5-hint-1}
"You might want to look up documentation for `if_else()` for this question. To make life easier for you, use the condition `(mean_overall > 4)` within an `if_else()`, followed by the character outputs we want separated by commas."
```

```{r C5-hint-2}
"Use this within a `map_chr()`: `if_else((mean_overall > 4), 'I love this dept.', 'I do not like this dept.')`"
```
...
qscores1 %>% 
  mutate(gthan4 = map_chr(mean_overall, ~ if_else((mean_overall > 4), 'I love this dept.', 'I do not like this dept.')))
...
8) Next, let's turn our attention towards the `trains` dataset from the `PPBDS.data` package. The data set carries information regarding the preferences and characteristics of each voter involved in the survey. 

First, lets `group_by()` by the genders mentioned in the survey, and then proceed to calculate the median income for each gender. 

```{r, D1, eval = TRUE, echo = TRUE, exercise = TRUE}

x <- trains %>%
  
  # Use `group_by() on the gender variable`
...
9) Now, let's once more use `map_dbl` to calculate the mean of the `income` variable for each gender. Call this column the `mean_income` column. 

```{r, setup5, eval = FALSE, echo = FALSE}
x
```

```{r, D2, eval=TRUE, echo=TRUE, exercise = TRUE}
...
"Use `~ mean(.$income)` inside the `map_dbl()` function, taking values from the `data` tibble."
...
x %>% 
  mutate(mean_income = map_dbl(data, ~ mean(.$income)))
...
9) Let's now employ anonymous functions to convert the mean incomes for the two groups from USD to Pound sterling. The conversion is as follows: $1 USD = £0.79 Pound Sterling.  

The data from the previous question has been prepared for you. Name the new column `pound`, and use the following anonymous function `. * 0.79` following a tilda (`~`). 

```{r, setup5_5, echo = FALSE, eval = FALSE}

x 

```

Use `map_dbl()` for this purpose.

```{r, D2_5, exercise = TRUE, echo = FALSE, eval = TRUE}
...
x %>% 
  mutate(mean_income = map_dbl(data, ~ mean(.$income))) %>% 
  mutate(pound = map_dbl(mean_income, ~ . * 0.79))
...
10) Let's now use the same dataset for the purposes of calculating the range of ages for groups that identify as `Democrats` or `Republicans`. We have grouped and nested the data for you.

```{r, setup6, echo = FALSE, eval=FALSE}

trains <- PPBDS.data::trains %>% 
  group_by(party) %>% 
  nest()

```

Use the the anonymous function `(max() - min())` along with `map_dbl()` to construct the column `range_ages`. 

```{r, D3, exercise = TRUE, eval=TRUE, echo=TRUE}
...
"(max(.$age) - min(.$age)) - wrap the `map_dbl() function around this anonymous function to compute the range. Obtain values from the `data` tibble."
```


```{r, D3-solution}

# Use `(.$age)` inside the `max()` and `min()` function calculate the range of the ages
# for groups identifying with either party.

trains %>% 
  mutate(range_ages = map_dbl(data, ~ (max(.$age) - min(.$age))))
