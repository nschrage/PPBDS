...
<!-- Want to edit later, may be a good place to introduce useful style tools within ggplot2 (title, subtitle, caption, text, etc) -->
...
<!-- Add how to add key parts to a graphic (title, subtitle, caption), themes, ggtext, axis, axis adjustments... -->
...
<!-- Moved this section after all of the examples of graphs (scatterplots, linegraphs, histograms, boxplots, barplots) to add an additional section called "More Layers" or "Style" (to replace the facets section) -->

<!-- Good section to include other layers we can add besides just facets; not sure if facets are significant enough to warrant having an entire section on them. Should probably include other layers including plot title, axes labels, visual themes for the plots, and facets -->
...
Table \@ref(tab:viz-summary-table) summarizes the differences among these graphs. Using them, you'll be able to visualize the distributions and relationships of variables contained in a wide array of datasets. This will be even more the case as we start to map more variables to more of each `geom`etric object's `aes`thetic attribute options, further unlocking the awesome power of the **ggplot2** package.

```{r echo=FALSE, message=FALSE}
# The following Google Doc is published to CSV and loaded using read_csv():
# https://docs.google.com/spreadsheets/d/1vzqlFiT6qm5wzy_L_0nL7EWAd6jiUZmLSCFhDhztDSg/edit#gid=0

if(!file.exists("data/summary_table_ch2.rds")){
  summary_table_ch3 <-
  ...
  write_rds(summary_table_ch3, "data/summary_table_ch2.rds")
} else {
  summary_table_ch2 <- read_rds("data/summary_table_ch2.rds")  %>% 
    rename(` ` = `X1`)
}
...
<!-- DK: Need to add others to this list as we expand the chapter. How best to organize? -->
<!-- EF: Need to add tail() in this list as well? --> 
...
# Do we need all of these? Probably knitr, but not the others?
...
<!-- EWF: End Section 1 for Jessica --> 

<!-- EWF: Start of section 2 for Jessica --> 
## Factors
...
```
<!-- EWF: End section 2 for Jessica --> 

<!-- EWF: Section 3 for Jessica --> 
## Importing data {#csv}
...
<!-- EWF: Tentative end section 3 for Jessica. Do you want to include web scraping
or should we keep it here for now --> 