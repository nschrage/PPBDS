...
<!-- 2) `primary_06` as a function of `treatment` and `solo` and of their interaction. We will build up this model step-by-step, very similar to how we explored the effect of treatment in chapter 8. But we go deeper because  we are learning about interactions. Note that we are not using any continuous variables on the left side. That is saved for Chapter 10. Key thing is to go through all the themes.Rmd issues, at least until prediction. Note that this situation is different from Chapter 8 in that fitted values and predicted values are not the same thing! The fitted value, for a combination of values for treatment and solo, is something 0.30, meaning that 30% of the people in this bucket votes. But the predicted value must be 0 or 1. Either you voted or you didn't. This example is clearly causal and so you need a Rubin Table with 4 potential outcome columns. -->

<!-- 3) EDA of `cces`. This can be fairly quick. Again, we won't use all the variables. Only discuss the ones we do use. (Or maybe we just use shaming but with lots and lots of buckets we make, like year buckets and past voting pattern buckets and hh_size/no_of_names buckets?! Let's discuss this before you work on this section and the next.) -->

<!-- 4) approval as a function of a bunch of things. (Unless we use shaming.) Again, we are only using categorical values. This is very similar to shaming, but there are no potential outcomes which we care about. This is purely a prediction exercise.  The problem with this model is that it leads to overfitting and, therefore, poor predictions. So, we need to use rstanarm to make a "better" model. -->
