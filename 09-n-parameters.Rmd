...
Having created models with one parameter in Chapter \@ref(two-parameter) and two parameters in Chapter \@ref(three-parameters), you are now ready to make the jump to $N$ parameters. 
...
We will start off by using a subset of the `governors` data set from the **PPBDS.data** package. This data set looks at the lifespans candidates for governor from 1945 to 2012. It corresponds to the paper "Longevity Returns to Political Office" by Barfort, Klemmensen & Larsen (2019), and aims to find out whether winning an election affects the lifespan of politicians.
...
Now, let's continue with the EDA by running `sample_n()`. 
...
Another useful command in summarizing data sets during and EDA is `skim()` from the **skimr** package.
...
ch9_gov %>%
  ...
  geom_point()
```
Starting with the relationship between `alive_post` and `year`, we can see that the data is skewed right. Why might this be?

<!-- Add more discussion of this graph -->
...
<!-- This is a simple example. Only three parameters are estimated: intercept, beta and sigma. But step through them all slowly, discussing what they mean. Not sure on the best way to display this. Maybe we just ignore sigma? Note that the intercerpt is meaningless here. How many days do you expect to live after the election if you have been alive zero days? That is nonsense. -->

```{r}
tbl_regression(obj.gov.1, intercept = TRUE)
```

<!-- You could center alive_pre and then redo the regression, and then  -->

<!-- ch9_gov <- ch9_gov %>% mutate(alive_pre_centered = alive_pre - mean(alive_pre)) -->
...
<!-- Consider using: ch9_gov %>% filter(sex == "Female") %>% ggplot(aes(alive_pre, alive_post)) + geom_point() + geom_smooth(method = "lm") -->
