...
where `"API KEY"` is replaced with your API key.^[If you run the `census_api_key()` function with the option `install = TRUE`, it will save your API key in `.Renviron` so you don't have to remember it.]

To get data from the decennial census, you can use the function `get_decennial()`.^[You can also get data from the American Community Survey using the **tidycensus** package, using the similar `get_acs()` function.] How can we use `get_decennial()` to retrieve each state's population from the 2010 Census? It only takes three arguments to get the data we want:
...
- `variables` selects which Census variables you want.  Unfortunately, they have rather opaque names.  If you didn't know that "P001001" was the variable name for population, you can find that out either by using the `load_variables()` function in **tidycensus** to generate a tibble of variable names (described [here](https://walkerke.github.io/tidycensus/articles/basic-usage.html#searching-for-variables)) or by searching on [data.census.gov](https://data.census.gov) and downloading the results of the search; the variable name will be in the .csv file.
- `year` is the year.  `get_decennial()` can obtain data from the 1990, 2000, and 2010 Census.

```{r pop_code, eval = FALSE}
library(tidyverse)
...
```{r pop_object, echo = FALSE, message = FALSE}
library(tidyverse)

pop <- read_rds(path = "rds/maps_pop.rds")
glimpse(pop)
```
...
#               output = "wide",
#               geometry = TRUE) %>%
#   write_rds(path = "rds/maps_rural.rds")

rural <- read_rds(path = "rds/maps_rural.rds")
...
Spatial data with a defined CRS can either be vector or raster data. Vector data is based on points that can be connected to form lines and polygons.  It is located with in a *coordinate reference system.*  An example is a road map.
...
An older package, **sp**, lets a user handle both vector and raster data.  This class will focus on vector data and the **sf** package.  The main differences between the **sp** and **sf** packages are how they store CRS information. While **sp** uses spatial sub classes, **sf** stores data in data frames, allowing it to interact with **dplyr** methods we've learned so far. I encourage you to check out other spatial data analysis and modeling [classes](http://www.rspatial.org/) if you remain interested in this afterward.

### Shape files

R can handle importing different kinds of file formats for spatial data, including KML and geojson. We'll focus on shape files, which was created by ESRI in the '90s.  Though we refer to a "shape file" in the singular, it's actually a collection of at least three basic files: 

* .shp - lists shape and vertices
* .shx - has index with offsets
* .dbf - relationship file between geometry and attributes (data)

All files must be present in the directory and named the same (except for the file extension) to import correctly.  Thankfully, **tidycensus** will grab the geometric information from the Census shape file for you.

## Mapping with **tidycensus** and `geom_sf()`

<!-- Explain how to use tidycensus to get geographic information -->

<!-- Explain how to create blank map -->
...
<!-- Discuss ways to make map prettier -->
...
rural %>%
  ...
  scale_fill_viridis_c(option = "plasma",
                       direction = -1) +
  labs(fill = "Percent Rural") +
  theme_void()
```

### Adding back Alaska and Hawaii

<!-- Discuss how to show Alaska and Hawaii -->
...
#               geometry = TRUE,
#               shift_geo = TRUE) %>%
#   write_rds(path = "rds/maps_rural_shifted.rds")

rural_shifted <- read_rds(path = "rds/maps_rural_shifted.rds") %>%
  rename(state = NAME) %>%
  mutate(prop_rural = P002005/P001001,
         state = reorder(state, prop_rural))
```
