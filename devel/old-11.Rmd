...
Let's now write the equation for our regression lines, which we can use to compute our fitted values $\hat{y} = \widehat{\text{income}}$. Here is the final equation, which we will break down in a second.
...
\end{aligned}
$$
First, let's use an indicator variable, ${1}_{\text{is male}}$, that assigns a value of 0 or 1 based on whether the subject is male or not.
...
Let's put this all together and compute the fitted value $\hat{y} = \widehat{\text{income}}$ for female subjects. Since for female subjects $\mathbb{1}_{\text{is male}}(x)$ = 0, the previous equation becomes
...
Before we end this section, we explain why we refer to this type of model as an "interaction model." The $b_{\text{age,male}}$ term in the equation for the fitted value $\hat{y}$ = $\widehat{\text{income}}$ is what's known in statistical modeling as an "interaction effect."
...
gendermale_coefs %>%
  ...
  labs(x = "Age",
       ...
       title = "Predicting incomes with gender and age",
       subtitle = "As age increases, being male is more associated with higher incomes")
...
Similarly to the regression table for the interaction model from Table \@ref(tab:regtable-interaction), we have an `intercept` term corresponding to the intercept for the "baseline for comparison" female subject group and a `gendermale` term corresponding to the *offset* in intercept for the male subjects relative to female subjects. In other words, the `r if_else(knitr::is_latex_output(), "darker-colored", "red")` regression line corresponding to the female subjects has an intercept of 94053 while the `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` regression line corresponding to the male subjects has an intercept of 94053 + 25236 = `r 94053 + 25236`. Once again, since there aren't any subjects of age 0, the intercepts only have a mathematical interpretation but no practical one. 
...
Let's now write the equation for our regression lines, which we can use to compute our fitted values $\hat{y} = \widehat{\text{income}}$.
...
Let's put this all together and compute the fitted value $\hat{y} = \widehat{\text{income}}$ for female subjects. Since for female subjects the indicator function $\mathbb{1}_{\text{is male}}(x)$ = 0, the previous equation becomes
...
Let's say we now have a 36 year old female subject and a 59 year old male subject. What fitted values $\hat{y}$ = $\widehat{\text{income}}$ would our `earnings_model_interaction` yield?

We answer this question visually first for the female subject by finding the intersection of the `r if_else(knitr::is_latex_output(), "darker-colored", "red")` regression line and the vertical line at $x$ = age = 36. We mark this value with a large `r if_else(knitr::is_latex_output(), "darker-colored", "red")` dot. Similarly, we can identify the fitted value $\hat{y}$ = $\widehat{\text{income}}$ for the male subject by finding the intersection of the `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` regression line and the vertical line at $x$ = age = 59. We mark this value with a large `r if_else(knitr::is_latex_output(), "lighter-colored", "blue")` dot.

```{r echo=FALSE, warning=FALSE, fig.cap="Fitted values for two new subjects.", fig.height=4.7}
newpoints <- earnings %>% 
  slice(c(1, 5)) %>% 
...
What are these two values of $\hat{y}$ = $\widehat{\text{income}}$ precisely? We can use the equations of the two regression lines we computed:

* For all female subjects: $\hat{y} = \widehat{\text{income}} = 131906 - 128	\cdot \text{age}$
* For all male subjects: $\hat{y} = \widehat{\text{income}} = 86787 + 1544 \cdot \text{age}$

So our fitted values would be: $131906 - 128	\cdot 36 = 127298$ and $86787 + 1544 \cdot 59 = 177883$, respectively. 

Now what if we want the fitted values not just for these two subjects, but for all of the subjects in the `trains` experiment? Doing this by hand would be long and tedious! This is where the `augment()` function from the **broom** package can help: it will quickly automate the above calculations for all 115 subjects. Just like in the last chapter, we'll use the `.se.fit` column to construct confidence intervals around our predicted values.
...
#   slice(1:10) %>%
#   gt() %>%
#   tab_header(title = "Regression points (First 10 out of 115 subjects)")
```

The `augment()` function also returns the residuals $y-\hat{y}$.  Notice that there are both positive and negative residuals for both male and female subjects. 
...
knitr::include_graphics("11-continuous-response/images/credit_card_balance_regression_plane.png")
```
