...
<!-- First, it provides a general discussion about how we select models. What makes one model better than another? How do we decide which variables to include? How do we select one functional form over another?  -->

<!-- Second, the initial discussion --- which covers basic stuff that you might find in any regression textbook --- then morphs into a more formal discussion about model choice, but includes the full machine learning approach, with an emphasis on holdout samples. -->

<!-- Third, now that we understand the basic ideas of model choice, how do we actually do it? Answer: **tidymodels**! We need to provide a thorough tour of tidymodels, perhaps using a bunch of text from TMWR: https://www.tmwr.org. -->
