...
<!-- 1. Replace Texas data with qscores. Keep all that material. Add key items from style.Rmd, e.g., unmodelled variation. -->

<!-- 2. Use a different model, like loess, to solve the exact same problem. Go through the same review, but more quickly. -->

<!-- 3. How do we decide? lm() or loess() or something? Incorporate (i.e., copy and paste) chapter 14 material. Maybe not tidymodels, or maybe so. -->
...
1. The **tidyverse** "umbrella" [@R-tidyverse] package. 
2. The **broom** [@R-broom] package, which provides an easy way to deal with model objects.
3. The **skimr** [@R-skimr] package, which provides a simple-to-use function to quickly compute a wide array of commonly used summary statistics. \index{summary statistics}

Additional Packages For Advanced Section: 

4. The **rstanarm** package, which provides an interface to the statistical inference engine, Stan, for Bayesian Regression Modeling. 
5. The **tidybayes** package, which aids in formating Bayesian modeling outputs in a tidy manner and provides ggplot geoms for plotting. 
