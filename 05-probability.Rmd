...
<!-- 2) Add screen shot from https://projects.economist.com/us-2020-forecast/president of electoral vote forecast. This is p(electoral votes for the Democratic nominee) or, shorter p(ev). It is just like all our other p()'s. It is a distribution because all values are between 0 and 1 and the sum is one. But it does not come from either math or from (direct) experience. Instead it is the distribution --- colloquially, the posterior distribution --- which comes from our modeling process. But this p() is, conceptually, the same as all our other p()'s. But what is the equivalent of the mathematical distribution for electoral votes and the empirical distribution? Good question! For math, I think we simulate the electoral vote distribution which would result from each state being a 50/50 toss up. Completely unrealistic, but very mathematical. For the empirical probability distribution, just look at the outcomes for 1960 and after. (I realize that this only gives us 17 values --- all probably unique --- but that is the data we have!) -->
...
<!-- 5) Get rid of infer package, I think. Do we really need rep_sample_n()? -->
...
<!-- VZ: Is the mathematical distribution not predicting what the distribution should look like? If that's the case does it not overlap with the posterior distribution definition? -->
...
In the case of the coin toss, the posterior distribution changes depending on your beliefs. For instance, let's say your friend brought a coin to school and asked to bet you. If the result is heads, you have to pay them $5. This makes you suspicious; your posterior distribution would reflect this, meaning that the probability of heads would be a lot higher than the probability of tails.

A posterior distribution for a more complicated issue like electoral votes requires more nuanced expectations. See this projection from The Economist, which uses various assumptions to generate a forecast.
...
<!-- 1) Initial part of the chapter will begin with three examples, each of which shows us three different sorts of probability distributions: mathematical, empirical, posterior. The three examples are flipping one coin, rolling two dice and the distribution of electoral votes. We do all three types for a coin, then all three for two dice and, finally, all three for electoral votes. Recall that a probability distribution is a mathematical object which covers a set of outcomes. All distinct outcomes have chances of occurring between 0 and 1. The sum of all the chances equals 1. We use `p()` as our symbol for probability distributions, with context demonstrating which type of probability distribution --- mathematical, empirical, posterior --- it is. -->

<!-- There are three kinds of probability distributions: mathematical, empirical and posterior. There are *mathematical* distributions (which are mathematical formulas, using fair dice). There are *empirical* distributions (which are data). Compare math of two dice and empirical data resulting from rolling the pair 20 times. The probability distribution is the Platonic form. The empirical distribution will often look like the mathematical probability distribution, but it will rarely be exactly the same. The third type of distribution is a *posterior* distribution, which is a measures of your belief about things you can't see right now. You can have posterior distributions about things in the past --- How many electoral votes would Hilary Clinton have won if she picked a different VP? About things in the present --- What is the median height of Harvard students? And about things in the future --- How many electoral votes will Joe Biden win?  -->
...
Flipping a single coin 1,000 times generates an empirical probability distribution with a value of around 0.5 for both heads and tails. Notice that the sum of the probabilities is 1.
