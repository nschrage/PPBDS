...
<!-- Jake Berg: My first comment for the textbook: in chapter 5, the language "Two models," "Three models," etc. is a bit confusing because it's not really mentioned until the end of the chapter what the "models" actually are. This makes the chapter slightly confusing because the connection between the two models example (disease testing), three models example (balls from an urn), and n models example (flipping a coin of unknown probability) is a bit unclear. It would therefore be super helpful if there were an explanation of what "two models" means in the beginning of the two models section. -->
...
```{r, echo=FALSE, fig.cap="Watch the makers of these two models throw shade at each other on Twitter! Eliot Morris is one of the primary authors of the Economist model. Nate Silver is in charge of 538. They don't seem to be too impressed with each other's work . . ."}
...
that you are rolling two coins together, or that you only care about the sum of the two dice values.
...
Remember the notation we talked about earlier in the chapter. Whenever we are talking about a specific probability (represented by a single value), we will use p with a subscript. However, when we are referring to the entire probability distribution over a set of outcomes, we will use p with parentheses. For example, the probability distribution of a coin toss is p(coin).
...
As you may have noticed, if you calculated the value using the first method, you would believe that getting 10 or more heads is less likely than it really is. If you were to run a crooked casino based on these assumptions, you might end up losing money. It is very important to be careful about the assumptions you are making. We tossed a coin 20 times and got 8 heads. However, you would be worng to assume that $p_h$ = 0.4 just based on this result.
...
```{r, echo=FALSE, fig.margin=TRUE, fig.cap="Prudence."}
...
```{r, echo=FALSE, fig.margin=TRUE, fig.cap="Fortitude."}
