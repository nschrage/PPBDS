...
<!-- p should be p_h. Use **ggtext** package so that you can have p_h in a axis label.   -->


<!-- Add some math to Justice.  -->

<!-- Add Gary KIng video. -->

<!-- Fix scatter plot in disease example. -->
...
```{r, echo=FALSE}
knitr::include_app("https://www.youtube.com/embed/gJZYgLyjyIQ")
```
...
text(150,320, "p(Disease)",cex=.5)

text(150,200, "p(No Disease)",cex=.5)

text(340,410,"p(Positive | Disease)",cex=.5)

text(340,300,"p(Negative | Disease)",cex=.5)

text(340,90,"p(Negative | No Disease)",cex=.5)

text(340,220,"p(Positive | No Disease)",cex=.5)
...
pop <- tibble(patient_id = 1:100000, 
              positive_test = c(rep(1, 10),
                               rep(0, 98010),
                               rep(0, 990),
                               rep(1, 990)),
              have_disease = c(rep(0, 10),
                               rep(0, 98010),
                               rep(1, 990),
                               rep(1, 990)))

pop <- tibble(patient_id = 1:100000, positive_test = c(rep(0, 10),
                                                       rep(0, 98010),
                                                       rep(1, 990),
                                                       rep(1, 990)),
              have_disease = c(rep(1, 10),
                               rep(0, 98010),
                               rep(0, 990),
                               rep(1, 990)))
...
ggplot(pop, aes(x = as.factor(have_disease), y = as.factor(positive_test))) +
  ...
  labs(title = "Joint Unnormalized Empirical Distribution \n of Test Results and Disease Status",
    x = "Test Result",
    y = "Disease Status") +
  scale_x_discrete(breaks = seq(0,1,1), labels = c("Negative", "Positive")) +
  scale_y_discrete(breaks = seq(0,1,1), labels = c("Negative", "Positive"))

```

Here is a joint distribution displayed in 3D. Instead of using the "jitter" feature in R to unstack the dots, we are using a 3D plot to visualize the number of dots in each box. The number of people who correctly test negative in this city is 98010, far outweighing any of the other categories. There are 990 false positives, 990 true positives, and only 10 false negatives. This is why we can barely see the 3D bar coming from those sections.
...
ggplot(mydata, aes(x = results)) +
  ...
  labs(title = "Conditional Probability Distribution",
       ...
       x = "Disease Status",
       y = "Probability") +
  scale_x_continuous(breaks = seq(0,1,1), labels = c("Positive", "Negative")) +
  theme_classic()
...
Let's say you got zero white marbles. We would isolate the slice where the result of the simulation involves three black marbles and zero white ones. Here is the unnormalized probability distribution.
...
x %>%
  ggplot(aes(y = p, x = heads)) +
    ...
    labs(title = "Empirical Distribution of Number of Heads",
         subtitle = "Based on simulations with various values of p",
         x = "Number of Heads out of 20 Tosses",
         y = "Probability of Heads") +
  theme_classic()
...
The first *wrong* way to do this is assuming that p is certain because we observed 8 heads after 20 tosses. We would conclude that 8/20 gives us 0.4. The big problem with this is that you are ignoring your uncertainty when estimating p. This would lead us to the following code.
...
Third way is to sample from the actual distribution, which is a small dataset with just a few rows, but it includes both $p$ and the probability of each $p$. This also gives the correct answer.
...
As you may have noticed, if you calculated the value using the first method, you would believe that getting 10 or more heads is less likely than it really is. If you were to run a crooked casino based on these assumptions, you might end up losing money. It is very important to be careful about the assumptions you are making. We tossed a coin 20 times and got 8 heads. However, you would be worng to assume that p = 0.4 just based on this result.
...
The PPBDS contains four important themes that are relevant in all chapters: Prudence, Temperance, Fortitude, and Justice. As you move through the textbook, you will be building on and refining these concepts.
...
The first theme of prudence is about whether the numbers we have capture the concepts we care about. We should be concerned about whether our data and models are representative of the problem.

Parameter uncertainty is a reason to exercise prudence. Suppose your friend sets up a pseudo-casino in the playground of the neighborhood school. She's offering everyone the opportunity to play probability games and bet money. The first game is simple: coin tosses. If the coin lands on heads, you get five dollars. If it lands on tails, you pay three dollars.
...
```
Data science involves words, math, and code. Code is the most important element. Turning an idea into code requires *fortitude*.

Fortitude helps us account for unmodeled variation. Recall the following equation.

$$outcome = model + what is not in the model$$
```{r, echo=FALSE, fig.margin=TRUE, fig.cap="De Finetti."}
...
A Bernouli distribution is where $n = 1$. 
...
The first question is whether the models we generate while calculating probabilities are predictive or causal. Knowing the difference helps us fairly interpret the data.
