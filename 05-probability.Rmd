...
```{r, echo = FALSE}
# Do a simulation in which you count what you get.

# x <- tibble(results = rbinom(n = 10000, size = 20, p = 0.8))


# Look at the counts. This is just the results. 

# ggplot(data = x, aes(results)) +
#   geom_bar()

# Transform the results so that it is a probability distribution. But you really
# don't have to turn this into a probability distribution until the end. You can
# just keep this as the results of the experiment.

# x %>%
#   group_by(results) %>% 
#   summarize(total = n(), .groups = "drop") %>%
#   mutate(prob = total/sum(total)) %>% 
#   ggplot(aes(results, prob)) +
#     geom_col()

# Do that for 9 values of p from 0.1 to 0.9. But this is not convenient.

# tibble(p = seq(0.1, 0.9, 0.1)) %>% 
#   mutate(experiment = map(p, ~ rbinom(n = 10000, size = 20, p = .))) %>% 
#   mutate(mean_heads = map_dbl(experiment, ~mean(.)))

# This is convenient because tidy data is convenient for plotting.

# x <- tibble(p = rep(seq(0.1, 0.9, 0.1), 10000)) %>% 
