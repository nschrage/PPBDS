...
<!-- We need to draw a tighter connection between the probability section and the 2/3/N models sections. Each of these features the creation, by hand, of a joint unnormalized distribution. In each case, we then take a slice of that joint distribution p(models, data) in order to get the conditional distribution p(models | data). Explain this more clearly each time, using this language.  Example:
...
<!-- More animations. Create a movie which swoops into the joint distribution, takes a slice, remove the other slices, and then normalizes. -->
...
<!-- There seems to be an intermittent problem in which terms like p_h do not show correctly in the html. Why!? Key html snippets look like 

<span class="math inline">\(p_t\)</span>

Maybe this just works or not depending on the browser? Or something else? Potentially related:

https://github.com/rstudio/rmarkdown/issues/1724
-->
