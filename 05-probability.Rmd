...
<!-- 0) Section headings are in sentence case (with only the first word capitalized, unless it is something always capitalized) rather than title case, which is what they are now. Section heading do not end with a period. -->

<!-- 1a) Add 4th sub-subpart to first part explaining Joint Distributions. Do this at the end of probability distribution part.  Example is one die with 50% likelihood of coming up 6 (and other faces at 10%) and second die 50% likelihood. Don't only care about the total. In this situation, you need a 3d plot. p(A, B) is p(Result of Die 1, Result of Die 2). Just one empirical plot of 100 rolls. Almost done, making 3D graph right now VZ -->
...
ggplot(mydata, aes(x = electoral_votes)) +
  geom_histogram(aes(y=..count../sum(..count..)), bins = 100, fill = "gray", color = "white") +
  labs(title = "Empirical Probability Distribution",
