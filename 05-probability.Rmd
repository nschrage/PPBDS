...
<!-- 1) Add 538 data in a plot that looks like our other plots. Add 538 screen shot. Replace Economist current screen shot with today's.  -->

<!-- 2) Cut Joint distribution discussion from Tree Diagrams. Move some (?) of that material to earlier discussion, if you want. -->
...
knitr::include_graphics("05-probability/images/economist_posterior_election.png")
...
Now that we understand both $p_A$ and $p_{A|B}$, let's talk about what it means to calculate $p_{A, B}$. With $p_{A|B}$, we are considering the probability of A assuming event B has already occurred. $p_{A, B}$ is the chance that both events will occur at the same point in time.
...
pop <- tibble(patient_id = 1:10000, tested_positive = c(rep(1, 110), 
                                                        rep(0, 9890))) %>%
...
ggplot(pop, aes(x = as.factor(have_disease), y = as.factor(tested_positive))) + 
  ...
  geom_jitter(alpha = .4) +
  theme_classic() +
  labs(title = "Distribution of Disease Results in a Population of 10000",
