...
<!-- 1) Add 538 data in a plot that looks like our other plots. Add 538 screen shot. Replace Economist current screen shot with today's.  done VZ-->

<!-- 2) Cut Joint distribution discussion from Tree Diagrams. Move some (?) of that material to earlier discussion, if you want. done VZ-->

<!-- 3) Make 2 model case like N model case. Start with unnormalized. Finish with discussion of *What sort of distribution is this?* Need two paragraphs to explain.  1) It is conditional, only for people with with a positive text. But, really, all probability distributions are conditional on something. Even our coin and dice distributions are conditional on assumptions about fairness and all the other assumptions that went into constructing the joint distribution of models and data. 2) Is it empirical or mathematical or posterior? Well . . . all of them . . . Almost every time we make a claim about the world, it is a claim about our beliefs, what we think. We could be wrong! Your beliefs will differ. So, since almost all pds are posterior, we often leave out the word posterior!  -->

<!-- In other words, almost every probability distribution is conditional and posterior, so we leave out both words, as we generally will in this book. They are always implicit. -->

<!-- p_h everywhere -->

<!-- Can we replace Maria's animation with rayshader? OK to use three rayshaders to tell the story. -->

<!-- Add Four Cardinal Virtues section at end, with a subsection about each virtue, with the image, just like Cass.  -->


<!-- 3) Adding three model example. Situation is that there is a bag with two marbles: either both white, both black or one of each. Those are the three models. Then, you take (and then replace) one marble three times. Possible values you can get: three blacks, two blacks, one black, zero blacks. So, we have three models and four possible experimental results. This is the bridge between the simple Bayes example with two models (disease or no) and two possible experiment results (positive test or negative test) and the N model example. -->
...
ggplot(mydata, aes(x = sum)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth = 1, 
                 color = "white") +
  labs(title = "Mathematical Probability Distribution",
       subtitle = "Sum from Rolling Two Die",
       x = "Outcome\nSum of Two Die",
       y = "Probability") +
...
ggplot(mydata, aes(x = sum)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth = 1, 
                 color = "white") +
  labs(title = "Empirical Probability Distribution",
       subtitle = "Sum from Rolling Two Die A Hundred Times",
       x = "Outcome\nSum of Two Die",
       y = "Probability") +
  scale_x_continuous(breaks = seq(0,12,1), labels = 0:12) +
  theme_classic()
...
The *posterior distribution* for rolling one dice a hundred times will vary based on your expectations. If you take the dice from your Monopoly set, you have reason to believe that the mathematical distribution is true. However, if you walk into a crooked casino and a host asks you to play craps, you might be suspicious. In craps, a come-out roll of 7 and 11 is a "natural" and the you win. You might expect those numbers to very rarely occur. Meanwhile, a come-out roll of 2, 3 is bad for someone betting what's known as the "Pass line." You might also expect values like 2 and 3 to occur more frequently.

```{r, echo=FALSE}

# Flipping a dice 100 times

mydata <- tibble(results = c(rep(2, 17),rep(3, 17),rep(4, 7),rep(5, 7),
                             rep(6, 7),rep(7, 5),rep(8, 7),rep(9, 7),
                             rep(10, 7),rep(11,5),rep(12,7)))

ggplot(mydata, aes(x = results)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth = 1, 
                 color = "white") +
  labs(title = "Posterior Probability Distribution",
       subtitle = "Your Belief About Rolling Two Die at a Crooked Casino",
       x = "Outcome\nSum of Two Die",
       y = "Probability") +
  scale_x_continuous(breaks = seq(0,12,1), labels = 0:12) +
  theme_classic()
...
It would be incredibly naive to create the following *mathematical distribution*. We create a distribution by running a simulation 10,000 times, where the chances of the Democratic candidate winning any given electoral vote is 0.5.
...
We know that campaign platforms, donations, charisma, and many other factors will contribute to a candidate's likeability. Elections are more complicated than coin tosses. Some political scientists may try to create algorithms to calculate mathematical probability distributions.
...
posterior_prez <- read.csv("./05-probability/data/election-forecasts-2020/prez_1.csv") %>%
  select(c(9,11))

ggplot(posterior_prez, aes(total_ev,evprob_chal)) +
  geom_bar(stat='identity') +
  labs(title = "Posterior Probability Distribution Based on FiveThirtyEight Data",
       subtitle = "Presidential Elections",
       y = "Probability",
       x = "Outcome\nElectoral Vote Total for Democratic Candidate") +
  theme_classic()
...
Here is a posterior from the Economist website (also from August 13, 2020). This looks confusing at first because the data analysts have chosen to merge the axes for Republican and Democratic electoral votes. We can tell that the Economist is quite optimistic about Biden's odds in the election compared to Trump's.

```{r, echo=FALSE, fig.fullwidth = TRUE, caption = "Economist Posterior from August 13, 2020"}
...
```{r, echo=FALSE, fig.fullwidth = TRUE, caption = "Economist Posterior from August 8, 2020"}

knitr::include_graphics("05-probability/images/joint_distribution.png")
...
How would you express this in statistical notation? p(A|B) is the same thing as the probability of A *given* B. $p_{A|B}$ essentially means the probability of A *if* we know for sure that B is true. You should also remember that $p_{A|B}$ is not the same thing as $p_{B|A}$.
...
The immediate, but *wrong*, answer would be 99%. Why is this wrong? Isn't 99% the accuracy of the test? To understand, we're going to abandon probability as a *proportion* (i.e. 99%) and think of it in *counts*. 

Imagine you're part of a population of 10000 people. Now, the frequency of the disease is 1%. In terms of a count, that means 100 people have this disease of the 10000 in the population. The test would probably (99% chance) correctly identify these individuals, meaning that around 99 will test positive. However, a test with an accuracy of 99% would also identify 100 other perfectly healthy people out of 10000 people as sick! That means that the number of people who would test positive for the disease is 199 but *only 99 of them have the disease*. So, in simple terms, the probability of you having the disease given a positive test is $99\div200$!

Here is an unnormalized distribution showing the number of true positives you would expect out of around 199 anticipated positive tests. As you can see, the y-axis displays the count instead of the probability, making this graph unnormalized.

```{r, echo = F}

x <- tibble(disease = rbinom(n = 199, size = 199, p = (99/200)))

x %>%
  group_by(disease) %>%
  summarize(total = n(), .groups = "drop") %>%
  ggplot(aes(disease, total)) +
  geom_col() +
  labs(title = "Distribution of the Number of Having the Disease Given a Positive Test",
       subtitle = "Comparing Two Models",
       x = "Outcome \nNumber of People with the Disease",
       y = "Probability") +
  theme_classic()

```
Here is the normalized version of the above graph, with the y-axis displaying the probability of each outcome.
...
```
What kind of distribution are you looking at? The first feature to notice is that the graph is conditional since we are only displaying cases where the person has already tested positive. The graph is asking, given the positive test, how many people actually have the disease. It may seem odd to zoom in on only those who have tested positive. However, in reality, all probability distributions are conditional on something. Even our coin and dice distributions are conditional on assumptions of fairness. In constructing joint distributions, we made assumptions about just how crooked the die at the crooked casinos were (ex. that one dice would return 5 50% of the time while the other would return 6 50% of the time).

The next question: is the distribution you see displayed above mathematical, empirical, or posterior? Well, technically all of them! Everytime we make a claim about the world, it is technically a claim about our beliefs. We could definitely be wrong and beliefs differ amongst people. In the case of the rare disease called $X$, we believe that the illness only affects 1% of the population. Since almost all probability distributions are posteriors in some way, shape, or form, we often leave out the word "posterior" when we call something a probability distribution. Up to now, we have distinguished between the three -- however, it is good to know that there is actually significant overlap.

However, just because the probability of having the disease is 1% and the test is 99% accurate, it doesn't mean that these are the exact numbers that always work out in the real world. That is, for a *sample* of 100000, a disease frequency of 1% doesn't necessarily mean that 100 people have the disease. It could be 101. Or 98. Or 105! Given that uncertainty, we model. We built an understanding of the world and now we can express it in counts. We can use our model to understand what the implications of an event (testing positive) are on our belief that we have the disease.

Let's visualize our two model distribution.

```{r disease, fig.align="center", echo = FALSE}

pop <- tibble(patient_id = 1:10000, positive_test = c(rep(1, 199),
                                                       rep(0, 9801)),
              have_disease = c(rep(1, 99),
                               rep(0, 9900),
                               rep(1, 1)))
...
ggplot(pop, aes(x = as.factor(have_disease), y = as.factor(positive_test))) +
  geom_point() +
  geom_jitter(alpha = .4) +
  theme_classic() +
  labs(title = "Distribution of Disease Results in a Population of 100000",
    x = "Did you test positive?",
    y = "Do you have the disease?")
...
# all_white <- tibble(results = c(rep(3, 1000)), model = c(rep(1, 1000)))
# 
# all_black <- tibble(results = c(rep(0, 1000)), model = c(rep(0, 1000)))
# 
# mixed_bw <- tibble(results = sample(0:3, size = 1000, prob = c(0.5, 0.5), 
#                                     replace = TRUE), model = c(rep(0.5, 1000)))

# sample(0:3, size = 1000, prob)
