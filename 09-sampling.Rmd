...
<!-- Redo all the photos over the summer. Put the data someplace besides Modern Dive. Where would that be? I like the idea of Google sheets. Or is that not robust enough? Maybe make the bowl in the code? Does it matter if students already know that there are 40% red? Perhaps that is a feature rather than a bug.  -->

<!-- Revisit Albert's functions. Is it OK that bowl is just treated as a global variable? Or should it be passed in as an argument. Also, are two functions really necessary? -->

<!-- Can we get rid of rep_sample_n? I hate to use any aspect of the infer package. -->

<!-- A big point of confusion is the difference between random sampling and random assignment. -->
...
As we observed in Figure \@ref(fig:comparing-sampling-distributions), as the sample size increases, the variation decreases. In other words, there is less variation in the 1,000 values of the proportion red. So as the sample size increases, our guesses at the true proportion of the bowl's balls that are red get more precise. 
...
```{r}
use_shovel <- function(x) {
  bowl %>% rep_sample_n(size = x, reps = 1000)
}

use_shovel(25)
```

See that we now can create the object `virtual_samples_25` with the code `virtual_samples_25 <- use_shovel(25)`.  This is far more succinct than our previous code, and it allows us to use a shovel of any size we'd like.
...
```{r, eval = FALSE}
virtual_prop_red_25 <- prop_red(25)
virtual_prop_red_50 <- prop_red(50)
virtual_prop_red_100 <- prop_red(100)
```
...
```{r}
shovels <- shovels %>%
  mutate(use_shovel_results = map(shovel_size, use_shovel),
         prop_red_results = map(shovel_size, prop_red))
...
shovels_100 <- tibble(shovel_size = 1:100) %>% 
    mutate(use_shovel_results = map(shovel_size, use_shovel),
         prop_red_results = map(shovel_size, prop_red),
         prop_red_sd = map_dbl(prop_red_results, ~ pull(., prop_red) %>% sd()))
...
Let's now switch gears to a more realistic sampling scenario than our bowl activity: a poll. In practice, pollsters do not take 1,000 repeated samples as we did in our previous sampling activities, but rather take only a *single sample* that's as large as possible.
