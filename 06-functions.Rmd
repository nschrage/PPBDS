...
library(tidyverse)
tibble(new_col = list(1:2, 3:4)) %>%
  str()
```


(By the way, `str()` is a helpful function for looking at the contents of a tibble with list columns.)
...
a. Create a tibble named `x` with two variables: `replication` and `throws`. `replication` is an integer from 1 to 10. `throws` is a list column, each element of which is three throws of the dice pair, i.e., the result of calling `roll_dice(n = 3)`.
