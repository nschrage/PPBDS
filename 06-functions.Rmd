...
Lists are very flexible.  For example, each element of a list can contain its own data types:
...
`col_1` contains the strings "Alice and Bob" and "Carol and Dan".  It may be more useful to present these as vectors of names, without the annoying "and" in between.  That's exactly what we do with `str_split()` in this example.  After using `str_split()` within a mutate, we have created a new column, and that new column is now a list.  We have a list column!

This is often how we will go about creating list columns.  

```{r gapminder_list_column}
gapminder %>%
  group_by(year) %>%
  summarize(lifeExpQuantile = list(quantile(lifeExp)))
```
...
```{r map_list_col}
tibble(col_1 = list(1:2, 3:4)) %>%
  mutate(col_2 = map_dbl(col_1, mean)) %>%
  str()
```
...
So, let’s practice `map_*` functions and list columns! (If I were going to give a lecture on these topics, it would look like [this one](https://resources.rstudio.com/webinars/how-to-work-with-list-columns-garrett-grolemund).) We recommend that you follow along step by step so that you understand how each part works.

We will start by creating two functinos: `dice()` and `roll_dice()`.  `dice()` will roll a single 6-sided die and `roll_dice()` will roll a pair of dice as many times as the user specifies.
...
b. Add a variable to `x` called `first_seven` which is TRUE if the first roll in `throws` is a 7. I recommend the use of “anonymous functions,” which you can read about [here](https://coolbutuseless.github.io/2019/03/13/anonymous-functions-in-r-part-1/).
...
x %<>% 
  mutate(first_seven = map_lgl(throws,
                               function(x) ifelse(7 == x[[1]], TRUE, FALSE)))
```
...
```{r a_winner}
x %<>% 
  mutate(a_winner = map_lgl(throws, 
                            function(x) ifelse(any(c(7, 11) %in% unlist(x)), TRUE, FALSE)))
```
...
surprised <- tibble(replication = 1:10000,
  ...
  mutate(perfection = map_lgl(throws,
                              function(x){ifelse(all(unlist(x) %in% c(7, 11)), TRUE, FALSE)}))
...
f. Your friend proposes the following bet. You will roll a pair of fair dice 10 times. Side A gets the second highest of the first 4 rolls. Side B gets 1 plus the the median of the remaining 6 rolls. Complete these Mad Libs:

```{r a_vs_b}
bet <- tibble(replication = 1:10000,
            throws = map(1:10000, ~ roll_dice(n = 10))) %>% 
  mutate(A = map_dbl(throws, function(x){sort(unlist(x)[1:4])[3]})) %>% 
  mutate(B = map_dbl(throws, function(x){1 + median(unlist(x)[5:10])})) %>% 
  mutate(winner = case_when(A > B ~ "A",
                            B > A ~ "B",
                            TRUE ~ "tie"))
...
The odds of a tie are approximately 11%.
