...
<!-- Write unfinished end of 06-functions.Rmd, portion devoted to list columns and map_ functions.  See https://r4ds.had.co.nz/many-models.html for a discussion about list columns. This topic is also difficult, first because we are teaching it in week 5 of the semester. Second, because it is hard! Third, because we only need to understand a subset of its usage --- the part that is useful for simple simulation exercises. R4DS is good: https://r4ds.had.co.nz/iteration.html.  RStudio Webinar titled "[How to Work with List Columns](https://resources.rstudio.com/webinars/how-to-work-with-list-columns-garrett-grolemund)" by Garrett Grolemund is useful. Maybe the this purrr tutorial: https://jennybc.github.io/purrr-tutorial/index.html? -->

<!-- 1) Begin by describing a list column with simple examples, just like https://r4ds.had.co.nz/many-models.html#list-columns-1. Just the simplest examples. Nothing about nest() yet. Simplest way to create a list column is by using mutate(new_col = list(1, 2, 3). These examples can all be tibbles with one or two rows. -->

We are now going to learn about [*list columns*](https://r4ds.had.co.nz/many-models.html#list-columns-1), a powerful tool that we will be using throughout the book.  A list column is a column of your data in which each entry is a list rather than a single value.  (A [list](https://adv-r.hadley.nz/vectors-chap.html#lists) is just a special kind of vector.) The best way to understand this is by creating one yourself:
...
(`str()` is a helpful function for looking at the contents of a tibble with list columns.)

Here we see that our tibble has one column (`new_col`) and two observations.  The first observation is a list that contains a vector of the numbers 1 and 2 and the second observation is a list that contains a vector of the numbers 3, 4, and 5.  
...
Now our first observation consists of the integers 1 and 2 while the second is a charcter vector containing "Alice" and "Bob".  Note how we wrapped "Alice" and "Bob" in the function `c()` in order to make it clear that this was a vector.  If we hadn't done that, we would have three observations: the vector with 1 and 2, "Alice", and "Bob":
...
<!-- 2) Then show how any function which returns more than one value can be used to create a list column. The simplest example uses str_split, as in https://r4ds.had.co.nz/many-models.html#creating-list-columns. Use the same approach. The next step is to use a group_by() so that there are "enough" values to run summary() or quantile() on. Use the gapminder data. -->
...
`col_1` contains the strings "Alice and Bob" and "Carol and Dan".  It may be more useful to present these as vectors of names, without the annoying "and" in between.  That's exactly what we can do with `str_split()`:
...
After using `str_split()` within a mutate, we have created a new column, and that new column is now a list.  We have a list column!
...
```{r continent_summary, eval = FALSE}
gapminder %>%
...
<!-- 3) What can we do with a list column? That is tricky! Lists are hard to work with. But the most natural thing to do with a list is to feed it to a map_* function. So, introduce map_ functions (which means we need the purrr package.) We can also use map functions to transform a list into a single variable. Point out map_lgl and friends. -->
...
Let's say that you wanted to generate 10 random integers between 1 and 10 and then add 1 to each of them.  You could accomplish this with a `for()` loop:

```{r for_loop_example, eval = FALSE}
...
Don't worry if this syntax doesn't make sense to you -- it isn't the best way to do it!  We should instead create a function that adds 1 to a number.  Then, we can use `map()` to apply this to our sample:

```{r map_simple, eval = FALSE}
x <- sample(1:10, size = 10, replace = TRUE)

increment <- function(x) x <- x + 1

# The map() function is part of the purrr package, which you will have loaded if
# you have loaded the tidyverse.

x <- map(x, increment)
```

This is much easier to read than the `for()` loop code  What happened is that `map()` took the function `increment()` and applied it to each element of `x`.  `map()` applies the same function to every element of the object.

The syntax can get even simpler than this if we use ["anonymous functions."](https://coolbutuseless.github.io/2019/03/13/anonymous-functions-in-r-part-1/).  The following code snippet shows two ways to use anonymous functions.  The first one, using base R, creates a function within `map()` using `function()`.  The second, using the purrr package of which `map()` is a part, uses the `~` operator and then uses `.` to represent the current element:


```{r map_anonymous, eval = FALSE}
x <- sample(1:10, size = 10, replace = TRUE) %>%
  map(function(x) x + 1)
...
The latter shorthand is very convenient once you get used to it.  Note that we are piping `x` directly into `map()`, which we can do because like other tidyverse functions, `map()` takes its data as the first argument.

We called these `map_*` *functions* (plural) before.  You may have noticed that `map()` always returns a list.  If you know the desired output of `map()`, you can have it return a different kind of vector:
...
So, since in our above code, we are outputting a number, we can use `map_dbl()` instead of `map()`:^[You may wonder why we aren't using `map_int()` instead.  We could, but we'd have to write the code a little differently.  R uses `L` to mark integers, so we'd have to say `map_int(~ . + 1L)`.]

```{r map_dbl, eval = FALSE}
x <- sample(1:10, size = 10, replace = TRUE) %>%
  map_dbl(~ . + 1)
```
...
<!-- 4) Now that we understand map_* functions, we can also use them to create list columns. In fact, working with a chain of map_* functions, each one taking as input a list column and producing a new list column as output is a very natural process, similar to piping a tibble from one dplyr function to another. Make that analogy clear. -->

Now that we understand `map_*` functions, we can also use them to create list columns.  We'll use the `weather` dataset in the nycflights13 package.


```{r map_list_col, eval = FALSE}
...
weather %>%
  ...
  mutate(temps_C = map(temps_F, ~ (. - 32) * 5/9),
         ...
         quantile_C = map(temps_C, quantile, na.rm = TRUE),
         top5_C = map(temps_C, ~ sort(unlist(.), decreasing = TRUE)[1:5]))
```

<!-- 5) Throughout we are leaving lots of stuff out. No broom, for example. We are purposely keeping to the simplest possible use, which involves simulation games of various kinds. I don't think we need nest() or unnest() at this stage. -->

### Practice with `map_*` functions and list columns

<!-- 6) End with problem set 5, question 5 from last semester, or something similar. These are the key concepts I want students to have. Steal that example for this chapter and walk students slowly through it, with extra backgound information. At the end (maybe?), show how the use of tibble() to start is a hack which can be replaced with imap. -->

Let’s practice `map_*` functions and list columns! (If I were going to give a lecture on these topics, it would look like [this one](https://resources.rstudio.com/webinars/how-to-work-with-list-columns-garrett-grolemund).) We recommend that you follow along step by step so that you understand how each part works.

We will start by creating two functions: `dice()` and `roll_dice()`.  `dice()` will roll a single 6-sided die and `roll_dice()` will roll a pair of dice as many times as the user specifies.

```{r roll_dice_function}
dice <- function() {
  sample(1:6, 1)
}
...
roll_dice <- function(n = 1) {
  ...
  # Return a vector where each element of the vector is the result of rolling
  # two dice.
...
a. Create a tibble named `x` with two variables: `replication` and `throws`. `replication` is an integer from 1 to 10. `throws` is a list column, each element of which is three throws of the dice pair, i.e., the result of calling `roll_dice(n = 3)`.  Thus, our tibble will have ten rows and two columns.

```{r throw_dice}
x <- tibble(replication = 1:10,
            throws = map(1:10, ~ roll_dice(n = 3)))
```

b. Add a variable to `x` called `first_seven` which is TRUE if the first roll in `throws` is a 7.

```{r first_seven, message = FALSE}
library(magrittr)

# The magrittr package allows us to use the compound assignment pipe operator,
# which (as its name suggests) both assigns and pipes

x %<>% 
  mutate(first_seven = map_lgl(throws, ~ ifelse(7 == .[[1]], TRUE, FALSE)))
```

c. Add a variable to `x` called `a_winner` which is TRUE if at least one of the three throws is a 7 or an 11 and is FALSE otherwise.

```{r a_winner}
x %<>% 
  mutate(a_winner = map_lgl(throws, ~ ifelse(any(c(7, 11) %in% unlist(.)), TRUE, FALSE)))
```

d. Run `str()` on `x` and show us the results.

```{r str_x}
str(x)
```

e. Calculate how “surpised” you should be if someone rolls three winners in a row. create a tibble with 10,000 rows. Include a `throws` list-column with three throws of our dice, just as in part a). Second, create a column called `perfection` which is TRUE if all three of the throws are either 7 or 11.

```{r surprised}
surprised <- tibble(replication = 1:10000,
            throws = map(1:10000, ~ roll_dice(n = 3))) %>% 
  mutate(perfection = map_lgl(throws, ~ ifelse(all(unlist(.) %in% c(7, 11)), TRUE, FALSE)))
...
f. Your friend proposes the following bet. You will roll a pair of fair dice 10 times. Side A gets the second highest of the first 4 rolls. Side B gets 1 plus the the median of the remaining 6 rolls. Which side is more likely to win?  What's the chance of a tie?

```{r a_vs_b}
bet <- tibble(replication = 1:10000,
            throws = map(1:10000, ~ roll_dice(n = 10))) %>% 
  mutate(A = map_dbl(throws, ~ sort(unlist(.)[1:4])[3]),
         B = map_dbl(throws, ~ 1 + median(unlist(.)[5:10])),
...
<!-- At some point, we need our own description of map_* functions and list columns. Till them, read about [iteration](https://r4ds.had.co.nz/iteration.html) and [list columns](https://r4ds.had.co.nz/many-models.html) in [*R for Data Science*](https://r4ds.had.co.nz). -->
