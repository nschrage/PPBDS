...
Here we see that our tibble has one column (`new_col`) and two observations.  The first observation is a vector of the integers 1 and 2 and the second observation is a vector of the integers 3, 4, and 5.  
...
Now our first observation consists of the integers 1 and 2 while the second is a character vector containing "Alice" and "Bob".

We wrapped "Alice" and "Bob" in `c()` in order to make it clear that this is a vector.  If we hadn't done that, we would have three observations: the vector with 1 and 2, "Alice", and "Bob":
...
After using `str_split()` within `mutate()`, we have created a new column, and that new column is a list.  We have a list column!
...
Let's say that you wanted to generate 10 random integers between 1 and 10 and then add 1 to each of them, storing the result in `x`.  You could accomplish this with a `for()` loop:
...
Don't worry if this syntax doesn't make sense to you -- it isn't the best way to do it!  We should instead create a function that adds 1 to a number.  Then, we can use `map()` to apply this to our sample.  `map()` comes from the `purrr` package, which you will have loaded if you have loaded `tidyverse`.  `map()` applies the same function to every element of the object:
...
This is much easier to read than the `for()` loop code.  What happened is that `map()` took the function `increment()` and applied it to each element of `x`.  

The syntax can get even simpler than this if we use [anonymous functions.](https://coolbutuseless.github.io/2019/03/13/anonymous-functions-in-r-part-1/)  The following code snippet shows two ways to use anonymous functions.  The first one, using base R, creates a function within `map()` using `function()`.  The second, using the `purrr` package (where we get `map()`), starts with the `~` operator and then uses `.` to represent the current element:
...
We called these `map_*` *functions* (plural) before.  If you printed `x` to the console after running the previous examples, you may have noticed that `map()` always returns a list.  If you know the desired output of `map()`, you can have it return a different kind of vector:
...
So, since our above code produces numeric output, we can use `map_dbl()` instead of `map()`:^[You may wonder why we aren't using `map_int()` instead.  We could, but we'd have to write the code a little differently.  R uses `L` to mark integers, so we'd have to say `map_int(~ . + 1L)`.]
