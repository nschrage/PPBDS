...
x <- house_ch12 %>% 
  ...
  rep_sample_n(size = nrow(house_ch12), replace = TRUE, reps = 10)

x
```

<!-- MB: had to change reps to 100 to run faster. change back later. -->
...
<!-- MB: Was this whole proof necessary? Has it been done in an earlier chapter? is the reminder worth it? -->
...
<!-- MB: Where did 0.657 come from? And does the second sentence mean to say "greater than in the Midwest", because I don't think that's what it means to say. -->
...
2. We get the regression table by applying the `tidy()` function from the **broom** package to `house_year_model`.  We'll print the `term`, `estimate`, `conf.low`, and `conf.high` columns.
...
2. We get the regression table by applying the `tidy()` function from the **broom** package to `house_interact_model`.  We'll print the `term`, `estimate`, `conf.low`, and `conf.high` columns.
...
regression_points %>%
  slice(1:10) %>%
  gt() %>%
  tab_source_note(md("First 10 out of 4,201 district-years"))
```
...
<!-- MB: These image links don't work, need to fix--> 
