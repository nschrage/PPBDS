...
virtual_heights %>% 
  ggplot() +
    geom_histogram(aes(x = height_90th, y=..count../sum(..count..)), binwidth = .9, fill = "red") +
...
<!-- Bootstrap creates posterior distirbution for your estimate of the unknown parametr: the 90th percentile of heights in the 2370 observations from nhanes -->
...
```{r}
do_the_bootstrap(heights_sample)
```
...
bootstrapchecks <- tibble(ID = 1:sims) %>%
  ...
  mutate(within_ci = map_lgl(ci, 
                             ~ ifelse(between(184.3, 
                                              ci[[1]][1], ci[[1]][2]), 
                                      TRUE, FALSE)))
