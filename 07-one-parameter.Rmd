...
<!-- Explain PPBDS.data and how to install it at start of chapter. Just a sentence or two, with `remotes::install_github("davidkane9/PPBDS.data")` in the sentence, along with note that, obviously, one needs to `library(remotes)` first. -->

<!-- Figure out the 5-8 subparts and the sub-subparts within each. This will evolve over time, of course.-->

<!-- Fix summarize warnings. I think you just need to provide a value for the `.groups` argument, perhaps just `.groups = "drop_last"`, which is the default. Of course, you also need to explain what this does. -->
...
<!-- a) We can see how good that confidence interval is because we have all the data. We know the hieght for all 5,000 people in the sample, so we can see what the true mean is. Did our 95% confidence interval include the mean? It should! -->
...
<!-- 2) Get rid of ## Warning: Removed 2 rows containing missing values (geom_bar). -->
...
# pennies_resample$mean_year <- map_dbl(1:1, ~get_year(pennies_resample$splits[[.]]))
...
```{r}
virtual_resample %>% 
  summarize(resample_mean = mean(year), .groups = 'drop')
```
...
virtual_resampled_means <- virtual_resamples %>% 
  group_by(replicate) %>% 
  summarize(mean_year = mean(year), .groups = 'drop')
...
perc_cis_80 <- perc_cis_80 %>%
  mutate(prop_red_results = map(boots, ~ group_by(., replicate) %>%
                                            summarize(prop_red = mean(color == "red"), .groups = 'drop')),
...
tibble(level = seq(0.50, 0.99, by = 0.01)) %>%
  ...
  mutate(width = upper - lower) %>%
  group_by(confidence_level) %>%
  summarize(mean_width = mean(width), .groups = 'drop') %>%
...
percentile_cis_by_n %>% 
  ...
  summarize(`Mean width` = mean(width), .groups = 'drop') %>% 
...
mythbusters_yawn %>%
  ...
  summarize(prop_yawn = mean(yawn == "yes"), .groups = 'drop')
```
...
mythbusters_yawn %>%
  ...
  group_by(replicate) %>%
  summarize(diff_prop_yawn = seed - control, .groups = 'drop')
```
...
sampling_distribution <- virtual_samples %>% 
  group_by(replicate) %>% 
  summarize(red = sum(color == "red"), .groups = 'drop') %>% 
...
se_boot <- bootstrap_distribution %>% 
  summarize(se = sd(prop_red), .groups = 'drop') %>% 
  pull(se)
```
