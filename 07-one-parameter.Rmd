...
<!-- 0. Question about male height in America in 2009. Ask some questions. Done -->

<!-- 1. Pennies. Calculate the estimate of the mean year and the confidence interval via bootstrap. Explain how lm(year ~ 1) %>% tidy(conf.int = TRUE) produces the same confidence intervals for the mean as the bootstrap does. We don't explain the math. Just show the trick. Note that this only works for a very small subset of problems. If we were curious about the median or the 90th percentile, we would have no choice but to use the bootstrap. Done -->

<!-- 2. Exploratory Data Analysis of nhanes. temp <- nhanes %>% select(year, gender, height). Explore all three variables. Note that there are NAs. Then restrict for our use in the models like: ch7 <- %>% drop_na() %>% filter(gender == "male", year == 2009) %>% select(height). In other words, the final data set just has one variable in it. Since we are only estimating a model with one parameter, we can't make use of richer data. -->

<!-- 3. height from the 2,475 men in nhanes in 2009. Pick one sample of size 40, without placement. sample_n(ch7, 40, replace = FALSE). This can go much faster. Two key differences from pennies. First, we know the truth, so we can examine whether or not the bootstrap works. (That is, we pull a sample of 40 and see if the bootstrap confidence interval from that 40 includes the true 75th percentile of the 2,475.) Second, note that only the bootstrap will allow us to do this. We can't use the lm() trick we learned about for pennies.-->
...
<!-- TA: Do you want me to do calculations or something for the standard error method or just explain it intuitively? -->
...
We can also run glimpse again to get a quick peek at what our subset of data looks like.
...
In addition to `glimpse`, we can use `skim` to look at our data as a tibble. 
...
Furthermore, it is always good to plot our data in order to explore it. Plotting our data can potentially show relations between variables and anything the general trends of our data. 
...
Let's go ahead and run `glimpse` and `skim` of our further subsetted data to get a basic idea of what our data will look like. 
...
skim(ch7)
```
Boom! So we can see that the mean of male height in 2009 is 168 cm and the 75th percentile of male height in 2009 is 180 cm from when we applied `skim` to our data. 
