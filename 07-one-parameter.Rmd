...
<!-- 2. Find a place for this quote somewhere as a sidemargin note. -->
...
<!-- 3. Get rid of all \index() TA: Done-->

<!-- 4. Get rid of library(moderndive) and pennies_sample. Make your own pennies sample by hand in the code. TA: Done-->
...
set.seed(9)
ch7_male <- nhanes %>%
  select(height, gender, year) %>%
  filter(gender == "male", year == 2009) %>% select(height) %>%
  drop_na()
```
...
The mean is `r ch7_male %>% pull(height) %>% mean()` and the 90th percentile is `r ch7_male %>% pull(height) %>% quantile(c(.90))`. We are looking at 90th percentile, which is the 237th tallest person. 
...
```{r}
set.seed(9)
heights_sample <- sample_n(ch7, 40, replace = FALSE)
```
...
Let's thus create our bootstrap samples. We already know how to create bootsrap samples from the `pennies_sample` section. Let's use the same code but obviously create the variables we are interested in, which are `height_mean` and `height_75th`. 
...
virtual_heights <- heights_sample %>%
  ...
  mutate(height_75th = map_dbl(heights, ~ quantile(., probs = 0.75))) 
virtual_heights
```
...
```{r}
set.seed(9)
mean_of_heightmeans <- virtual_heights %>%
  select(height_mean) %>%
  summarize(mean_of_means = mean(height_mean)) %>%
  pull() %>% 
  round(2)
mean_of_height75th <- virtual_heights %>%
  select(height_75th) %>%
  summarize(mean_of_75th = mean(height_75th)) %>%
  pull() %>% 
  round(2)
```

Thus, based on our bootstrap samples, the mean male height in 2009 is `r mean_of_heightmeans` and the 75th percentile of male heights in 2009 is `r mean_of_height75th`. It appears that we are very close to the actual estimates, which was 168 cm and 180 cm, respectively.
