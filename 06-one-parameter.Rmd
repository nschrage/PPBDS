...
<!-- 1) Devito image seems small. Is that because it is png? Do jpeg files automagically get scaled so that they are the correct size? Maybe put it as a side margin. DONE. -->

<!-- 2) Do the tutorial. Modify the tutorial. It is not bad, but, with map2, it is a bit much, I think. See the comments. DONE. -->

<!-- MF: I edited the tutorial, but left the mapping until we spoke. Would you prefer mapping be taken out altogether or would you prefer that I add mapping to the chapter and keep it in the tutorial? From what I understand, the tutorial is meant to be easy if you've read the chapter.  -->

<!-- 3) Start thinking about what we should do in the class. -->

<!-- MF: The in-person bead activity was helpful for seeing sampling in action and seeing a live distribution. Maybe there would be a way to create a survey for students on something simple (like the number of pets they have?) and then make that into a dataset, draw using rep_sample_n and creating a distribution that way. The participation component might help with engagement in the topic. Just an idea-- I need to contemplate ideas more... Trying to think of something that is fun but more challenging than the urn. -->

<!-- 0) Add sqrt(n) plot to show how standard error decreases. There is  ahandy stat_ for doing this in ggplot2. But I deleted it when told me you had done this already. NOTE: this works for me as is. Maybe because my version of R is old? -->
...
<!-- Create three-D plot, like Vivian does in chapter 5 with rayshader. Recommend having separate code which creates the plot, then save the png in images/ and then just read it in. DONE but I am working on getting rid of the axis labels in the slice rayshader (even though I took them out in ggplot, they are staying). -->
...
```{r, echo=FALSE, fig.cap="Mr. DeVito is correct: sampling *is* badass. Let's get started!", fig.margin = TRUE}
...
Our final question, once we have mastered sampling, will be: if we pull out 16 red beads with a sample of size 50, what is the distribution of possibilities for the total number of red beads in the entire urn?
...
`r margin_note("The .groups = drop_last command is summarize's default behavior. However, if you do not specify the .groups argument, an error message occurs. This is why it is included here.")`
...
As with our 33 groups of friends' tactile samples, there is variation in the resulting 33 virtual proportions red. Let's visualize this variation in a histogram in Figure 6.8.
...
We can now create the object `virtual_samples_25` with the code `virtual_samples_25 <- use_shovel(x = urn, size = 25, reps = 1000)`.  This is far more succinct than our previous code, and it allows us to use a shovel of any size we'd like.

```{r, echo=FALSE, fig.cap="That's right! Functions can be passed into more functions. How fun.", fig.margin = TRUE}
knitr::include_graphics("06-one-parameter/images/function_meme.png")
```
...
```{r, echo=FALSE, fig.cap="To any poets and philosophers confused about this: don't worry! It won't be on a problem set.", fig.margin = TRUE}
...
```{r, echo=FALSE, fig.cap="Fear not if you look like Spongebob after reading this section. We will re-cap right now!"}
...
```{r, echo=FALSE, fig.cap="We are almost done! Hang in there."}
...
```{r, echo=FALSE, fig.cap="Real life sampling is hard! In most scenarios, it's impossible to ask the entire population for their opinions."}
...
```{r, echo=FALSE, fig.cap="Sampling doesn't work if the data doesn't map to our research question!", fig.margin = TRUE}
knitr::include_graphics("06-one-parameter/images/tree_meme.png")
```
...
```{r, echo = FALSE}
sample_obama_preceptor <- tibble(ID = c("Citizen 1", "Citizen 2", "...", 
                                    "Citizen 10000", "Citizen 10001", "...", 
                                    "Citizen 49999999", "Citizen 50000000"),
                           approval_rating = c("?", "?", "...", 
                                               "?", "31", "...",
                                               "?", "?")) 

obama_preceptor <- sample_obama_preceptor %>% 
  mutate(ID = as.character(ID),
         approval_rating = as.character(approval_rating)) %>%
  gt() %>%
  cols_label(ID = md("**ID**"),
...
```{r, echo=FALSE, fig.cap="Listen to Kendrick. If we are too confident, our assumptions about the world will be false!"}
knitr::include_graphics("06-one-parameter/images/kendrick.png")
```
...
```{r, echo=FALSE, fig.cap="El Jefe's, a Harvard Square staple, is known for its large portions and delicious horchata.", fig.margin = TRUE}
knitr::include_graphics("06-one-parameter/images/el_jefes.png")
```

```{r, echo=FALSE, fig.cap="Felipe's, another staple, is said to have a more authentic experience and addicting churros.", fig.margin = TRUE}
...
### Shake Shack or Tasty Burger?

```{r, echo=FALSE, fig.cap="Shake Shack is known for its plethora of shake flavors, crinkle cut fries, and interaction-less checkout stands.", fig.margin = TRUE}
knitr::include_graphics("06-one-parameter/images/shake_shack.png")
```

```{r, echo=FALSE, fig.cap="Tasty Burger is famous for its late-night hours, huge vats of mustard, and beer (for those of drinking age).", fig.margin = TRUE}
knitr::include_graphics("06-one-parameter/images/tasty_burger.png")
``` 
...
```{r, echo=FALSE, fig.cap="A 3D image of our joint distribution created with Rayshader."}
...
Therefore, when answering our chapter's complex question, "what is our best guess at the number of red beads in the entire urn if we get a result of 16 red beads in a sample of size 50", there is not one singular answer. Our posterior distribution is just that: a distribution. From our sample, we have many different results for the total number of red beads in the entire urn. Certain totals of red beads, like the extremes close to 0 or 2400, are impossible due to our sample of 32%. Other totals, like 1000, occur far more frequently than other totals. 

This means that, while we can provide a range of possibilies (and we can know which of those possibilities occurs most frequently), we can never say that we know the total number of red beads with certainty. 

The key issue with the urn paradigm is that, unlike real-world data science, we *can* technically find the total number of red beads. We just have to perform that exhuaustive count that we've been so desperately avoiding! In the real-world, the "true" solution can never be known. This is why we require inference, and why tools like sampling are so helpful for making assumptions about the world around us. 
...
<!-- ELABORATE on fitted model & unmodeled variation here -->
