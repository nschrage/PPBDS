...
```{r}
interact_form <- formula(ideology ~ gender * race + state * income + age + education)
```

```{r eval=FALSE}
fit(lm_model, interact_form, ch11_training)
```
...
```{r}
lm_rec <- recipe(ideology ~ gender + race + state + income + age + education, data = ch11_training) %>%
...
```{r}
lm_model %>%
  fit(interact_form, data = ch11_training) %>%
...
set.seed(10)
fit(stan_model, interact_form, ch11_training) %>%
  tidy(conf.int = TRUE) %>%
...
```{r}
lm_model %>%
  fit(non_state_form, data = ch11_training) %>%
  predict(new_data = ch11_testing)
```
