...
edu_form <- update(age_gender_form,
                   ~ . + education)

interact_form <- update(basic_form,
                        ~ . + gender * race + income)
```
...
```{r}
nes_formulas <- tibble(formula = c(lm_formula,
                                   age_gender_form,
                                   demo_form,
                                   edu_form,
                                   interact_form),
                       group = c("Full model",
                                 ...
                                 "Demographic model",
                                 "Demographic and education model",
                                 "Demographic model with interaction"))
```
...
folds_metrics <- nes_formulas %>%
  mutate(metrics = map(formula, ~ fit_resamples(stan_model,
                                                recipe(lm_formula, data = nes_training),
                                                nes_folds) %>%
                         collect_metrics()))
```
