...
For this chapter, we will seek to answer the following question: How does age*gender affect ideology for residents of the four different regions of the U.S.?
...
Let's take a look at the first ten residual values.

<!-- EC: how to make augment work for stan_fit objects? It should work according to broom.mixed documentation.... -->

```{r eval=FALSE}
broom::augment(stan_fit$fit) %>%
  select(ideology, .fitted, .resid, gender, race, region, income, age, education)
```
...
Note that there is no tidy method for neural network models. This makes sense, as the terms are not necessarily additive. However, we can still view the predictions by using `bind_cols()`.

```{r}
bind_cols(
  predict(nnet_fit, new_data = ch11_training))
```
