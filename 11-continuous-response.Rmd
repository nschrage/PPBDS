...
Note that certain variables we would like to include are characters. We should convert them into factor types to indicate that they are categorical predictor variables.
...
Great! Now we can proceed with creating models to estimate respondents' ideology.
...
Keep in mind that, by dropping the `NA`s to create our cleaner dataset, we are creating an ideal Preceptor Table in which there are no unknown values. We are then using this ideal Preceptor Table to make predictions for new data. 

However, recall the virtue of Justice: **We want to make sure our models are as just and representative of the real world as possible.** As such, we will split our `nes_2016` data into two sets: a training set and a testing set. We can do so using the ***rsample*** function, `initial_split()`.
...
<!-- EC: Is there a better/more simple way to introduce MLP? -->
...
```{r this-is-not-knitting, message=FALSE}
lm_model %>%
  fit(interact_form, data = nes_training) %>%
...
<!-- DK: This fails because a factor variable is defined by all its levels. The `state` variable in new_people is not the same as the state variable you used to fit the model. You need to add all the missing levels when you create them. Something like:

"state" = factor(c("VA", "AL", "VA", "AL"), levels = levels(ch11$state))
