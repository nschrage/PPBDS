...
1) Add section on PDFs and their relationship with histograms for flipping die
- Rolling a dice 1 time gives us a flat histogram
- Rolling a dice 1000 times gives us a histogram with probability on the y axis, where all the values add up to 1
- Show that adding up the area under gives us a sum of 1.
- Introduce the notation P(coin) and P(1 dice) earlier
- Continuity is a lie, height is not continuous. The die are discrete, but height is continuous. So height is a sum of discrete things here.
- You technically can't measure continuous anything and even if you could, you can't put it on a computer
- Talk about the relationship between the height of buckets and the number of buckets.
...
knitr::include_graphics("images/donald_rumsfeld.jpg")
...
# Step 1: Creating the dice.

dice <- c(1:6)

# Step 2: Selecting the number of simulations.

sims <- 10000

# Step 3: Creating a tibble with the data from each simulation.

mydata <- tibble(dice_1 = sample(dice, size = sims, replace = TRUE),
                 dice_2 = sample(dice, size = sims, replace = TRUE))

# Step 4: Filtering out simulations where we don't have any 6's, then
# counting our remaining observations.

count <- mydata %>%
  filter(((dice_1 + dice_2)/2) %in% 3) %>%
  count() %>%
  pull(n)

# Step 5: Calculating probability, given by the number of observations with even
# numbers divided by the number of total observations.
...
# Let's create a column for the sum of the two die.

mydata <- mydata %>%
  mutate(average = (dice_1 + dice_2)/2)

# Now let's create a histogram with the y-axis re-scaled to have a maximum of 1.

ggplot(mydata, aes(x = average)) +
...
# As we increase the number of die we roll, our probability distribution looks
# more and more like a probability density function.

dice <- as.data.frame(dice)

# Creating an empty tibble to fill with sampling data in the next step.

updated_data <- data.frame()

# Creating samples of different numbers of die, from 1 die to 100 die. Sampling
# at each number of die sims = 10000 times.

for (i in 1:2) {
  
  new_data <- rep_sample_n(dice, size = i, replace = TRUE, 
                         reps = sims) %>%
    group_by(replicate) %>%
    summarize(average = sum(dice)/i)
  
  updated_data <- rbind(new_data, updated_data)
  
}

# ggplot(newdata, aes(x = average)) +
#   geom_histogram(aes(y=..count../sum(..count..)), binwidth = (1-(0.5*1/i)), 
#                  fill = "dark green", color = "white")
# 
# number_of_die <- 100
# 
# mydata <- rep_sample_n(dice, size = number_of_die, replace = TRUE, reps = sims)
# 
# newdata <- mydata %>%
#   group_by(replicate) %>%
#   summarize(average = sum(dice)/number_of_die)
# 
# ggplot(newdata, aes(x = average)) +
...
![](Maria_Drafts/color_red_combo.gif)
