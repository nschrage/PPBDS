...
<!-- 2. use some arguments mean of rnorm. -->

<!-- 3.check those arguments. sd must be positive -->

<!-- 4. pass in the number of each that you want. gets to dimensionality. you might require on one n variable. -->

<!-- 5. data generating mechanism. make their own posterior_linpred and posterior_predict. -->

<!-- 6. What are you returning? Can send back a vector, which can work well if called by mutate. But could also tend back a tibble, which (if you require a tibble to be sent in) would allow for pipes. -->
