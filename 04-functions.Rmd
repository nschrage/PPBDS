...
<!-- 1. rnorm(1) + runif(1) Done.-->

<!-- 2. use some arguments mean of rnorm. Done-->

<!-- 3.check those arguments. sd must be positive Done-->

<!-- 4. pass in the number of each that you want. gets to dimensionality. you might require on one n variable. Done-->

<!-- 5. data generating mechanism. make their own posterior_linpred and posterior_predict. Done-->

<!-- 6. What are you returning? Can send back a vector, which can work well if called by mutate. But could also tend back a tibble, which (if you require a tibble to be sent in) would allow for pipes. Done-->

<!-- 7. Eventually, you are building predict, but with variation. -->
...
Let's examine the `kenya` dataset from `PPBDS.data`. Make sure to load the library. What if we were interested in taking the difference between the highest poverty rate and the lowest poverty rate? First, develop some working code for interactive use, using a representative input. Use the `poverty` variable. Built-in R functions that will be useful: `min()`, `max()`, `range()`. 
...
```{r}
dgm <- function(n, mean, sd) {
  stopifnot(is.numeric(c(mean, n, sd)))
  runif(1) + rnorm(n, mean, sd)
}
```
...
dgm <- function(pie, cookie, cheesecake) {
  stopifnot(n >= 0,
            sd >= 0,
            is.numeric(c(pie, cookie, cheesecake)))
...
### Restore our data generating function
...
<!-- EC: Should we keep the NA section? It's now less relevant with the data generation mechanism stuff, but if we want to keep it we can use the `kenya` dataset. -->
...
qdiff5(kenya$poverty)
qdiff5(z)
qdiff5(z, na.rm = FALSE)
```

<!-- EC: The following section on ellipses is again not as applicable for the data generating function, and also is covered during map functions and list-columns. Is it ok to delete? -->
...
```{r restore-correct-func, echo = FALSE}
dgm <- function(n, mean = 0, sd = 1) {
  stopifnot(sd >= 0,
            n >= 0,
            is.numeric(c(mean, n, sd)))
