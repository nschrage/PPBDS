...
perc_cis_80 <- bowl %>%
  ...
  mutate(boots = map(data, ~ rep_sample_n(., size = 50, replace = TRUE, reps = 1000)),
         prop_red = map(boots, ~ group_by(., replicate) %>%
                          summarize(prop_red = mean(color == "red")) %>%
                          pull(prop_red)),
         lower = map_dbl(prop_red, ~ quantile(., probs = 0.1)),
         upper = map_dbl(prop_red, ~ quantile(., probs = 0.9)),
         confidence_level = "80%") %>%
  ungroup() %>%
  select(lower, upper, confidence_level)
...
perc_cis_n_25 <- bowl %>%
  ...
  group_by(replicate) %>%
  nest() %>%
  mutate(boots = map(data, ~ rep_sample_n(., size = 25, replace = TRUE, reps = 1000)),
         prop_red = map(boots, ~ group_by(., replicate) %>%
                          summarize(prop_red = mean(color == "red")) %>%
                          pull(prop_red)),
         lower = map_dbl(prop_red, ~ quantile(., probs = 0.025)),
         ...
         sample_size = "n = 25") %>%
  ungroup() %>%
  select(lower, upper, sample_size)
...
perc_cis_n_100 <- bowl %>%
  rep_sample_n(size = 100, reps = 10) %>%
  
  # For each one, construct a CI
  
  group_by(replicate) %>%
  nest() %>%
  mutate(boots = map(data, ~ rep_sample_n(., size = 100, replace = TRUE, reps = 1000)),
         prop_red = map(boots, ~ group_by(., replicate) %>%
                          summarize(prop_red = mean(color == "red")) %>%
                          pull(prop_red)),
         lower = map_dbl(prop_red, ~ quantile(., probs = 0.025)),
