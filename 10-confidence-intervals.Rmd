...
Specifically, we'll first take 30 different random samples of size $n$ = 50 balls from the urn. Then we'll construct 10 percentile-based confidence intervals using each of the three different confidence levels. 
...
```{r}
urn_confidence <- function(n = 50, level = 0.95) {
...
```{r}
perc_cis_80 <- urn_confidence(50, 0.80)
perc_cis_95 <- urn_confidence(50, 0.95)
perc_cis_99 <- urn_confidence(50, 0.99)
...
But this still isn't the best way.  Note that we have three objects we need to deal with.  Furthermore, we don't actually know that the names are accurate! Perhaps we made a mistake and created `perc_cis_99` with the code `perc_cis_99 <- urn_confidence(50, 0.95)`; our object name will now be misleading as to what's actually in the object.

Instead, let's store our results in a single tibble by using `map()` to create a list column.

```{r}
tibble(level = c(0.80, 0.95, 0.99)) %>%
  mutate(data = map(level, ~ urn_confidence(n = 50, level = .)))
```
...
perc_cis_n_25 <- urn_confidence(n = 25, level = 0.95)
perc_cis_n_50 <- urn_confidence(n = 50, level = 0.95)
perc_cis_n_100 <- urn_confidence(n = 100, level = 0.95)
...
tibble(n = seq(25, 200, by = 5)) %>%
  mutate(data = map(n, ~ urn_confidence(n = ., level = 0.95))) %>%
  unnest(data) %>%
  select(-n) %>%
...
<!-- DK: Check to see if this new sample matches the text. -->
...
<!-- There exists in many cases a formula that approximates the standard error! In the case of our `urn` where we used the sample proportion red $\widehat{p}$ to estimate the proportion of the urn's balls that are red, the formula that approximates the standard error is: -->

<!-- $$\text{SE}_{\widehat{p}} \approx \sqrt{\frac{\widehat{p}(1-\widehat{p})}{n}}$$ -->

<!-- For example, recall from `urn_sample_1` that Yohan and Ilyas sampled $n = 50$ balls and observed a sample proportion $\widehat{p}$ of 21/50 = 0.42. So, using the formula, an approximation of the standard error of $\widehat{p}$ is -->
...
<!-- Yohan and Ilyas are 95% "confident" that the true proportion red of the urn's balls is between 28.3% and 55.7%. Given that the true population proportion $p$ was `r p_red`, in this case they successfully captured the fish. -->
...
<!-- Let's revisit our 33 friends' samples from the `urn` from Subsection \@ref(student-shovels). We'll use their 33 samples to construct 33 theory-based 95% confidence intervals for $p$. Recall this data was saved in the `tactile_prop_red` data frame included in the **moderndive** package: -->
...
<!-- In Figure \@ref(fig:tactile-conf-int), let's plot the 33 confidence intervals for $p$ saved in `conf_ints` along with a vertical line at $p$ = `r p_red` indicating the true proportion of the `urn`'s balls that are red. Furthermore, let's mark the sample proportions $\widehat{p}$ with dots since they represent the centers of these confidence intervals. -->
