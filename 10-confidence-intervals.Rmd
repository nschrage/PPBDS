...
However, as described in Section \@ref(sampling-simulation), both the tactile and virtual sampling exercises are not what one would do in real life; this was merely an activity used to study the effects of sampling variation. In a real-life situation, we would not take 1,000 samples of size $n$, but rather take a *single* representative sample that's as large as possible. Additionally, we knew that the true proportion of the bowl's balls that were red was 37.5%. In a real-life situation, we will not know what this value is. Because if we did, then why would we take a sample to estimate it? 
...
We'll start by walking through the process for the 80% confidence intervals.  We begin by taking 10 random samples from the bowl of size $n$ = 50:
...
The moral of the story is: \index{confidence interval!impact of confidence level on interval width} **Higher confidence levels tend to produce wider confidence intervals.**  When looking at Table \@ref(tab:perc-cis-average-width) it is important to keep in mind that we kept the sample size fixed at $n$ = 50. Thus, all $10 \cdot 3 = 30$ random samples from the `bowl` had the same sample size. What happens if instead we took samples of different sizes? Recall that we did this in Subsection \@ref(different-shovels) using virtual shovels with 25, 50, and 100 slots.

### Fitting multiple models using `map()`

Note that we used the same code three times.  That means we should write a function!  Let's adapt the code from the last section to take 10 samples of an arbitrary size and construct confidence intervals of an arbitrary level:

```{r}
bowl_confidence <- function(n = 50, level = 0.95) {
...
```{r}
perc_cis_80 <- bowl_confidence(50, 0.80)
perc_cis_95 <- bowl_confidence(50, 0.95)
perc_cis_99 <- bowl_confidence(50, 0.99)
...
But this still isn't the best way.  Note that we have three objects we need to deal with.  Furthermore, we don't actually know that the names are accurate! Perhaps we made a mistake and created `perc_cis_99` with the code `perc_cis_99 <- bowl_confidence(50, 0.95)`; our object name will now be misleading as to what's actually in the object.

Instead, let's store our results in a single tibble by using `map()` to create a list column.

```{r}
tibble(level = c(0.80, 0.95, 0.99)) %>%
  mutate(data = map(level, ~ bowl_confidence(n = 50, level = .)))
```
...
Note that now that we have a function, `bowl_confidence()`, we can do this very easily!

```{r}
set.seed(9)


perc_cis_n_25 <- bowl_confidence(n = 25, level = 0.95)
perc_cis_n_50 <- bowl_confidence(n = 50, level = 0.95)
perc_cis_n_100 <- bowl_confidence(n = 100, level = 0.95)
...
tibble(n = seq(25, 200, by = 5)) %>%
  mutate(data = map(n, ~ bowl_confidence(n = ., level = 0.95))) %>%
  unnest(data) %>%
  select(-n) %>%
...
It turns out that we can, using the `lm()` function.  `lm()` stands for `l`inear `m`odel, and we will be using it a lot more in the next two chapters.  This function can get the mean of a variable using the following syntax:
...
The **broom** package makes it a lot easier to work with `lm()` model objects.  In particular, the function `tidy()` has the option `conf.int = TRUE`, which we can use to get a confidence interval:
...
These are not identical to our bootstrap confidence interval estimates, but they are similar.  Thus, if you are interested in constructing a confidence interval for a mean, you can use `lm()` and `tidy()` as a shortcut.
...
Recall that you learned about treatment and response variables in Chapter \@ref(rubin-causal-model).
...
See how now we have two rows per `replicate`, the first being `control` and the second being `seed`.  That means we can construct our difference in proportions by `group`ing `by` `replicate` and subtracting one `prop_yawn` from the other:
...
mythbusters_yawn %>%
  ...
  summarize(prop_yawn = mean(yawn == "yes")) %>%
  group_by(replicate) %>%
  summarize(diff_prop_yawn = prop_yawn[2] - prop_yawn[1])
```

Observe that the resulting data frame has 1,000 rows and 2 columns corresponding to the 1,000 `replicate` ID's and the 1,000 differences in proportions for each bootstrap resample in `diff_prop_yawn`.
...
```{r}
lm(yawn == "yes" ~ group, data = mythbusters_yawn)
```

There are two differences from what we've seen before.  First, since `yawn` is a character variable, we had to specify that our outcome is "yes" by using `yawn == "yes"`.  Second, since we are interested in the difference in means by group, we used `group` instead of `1` on the right side of the `~`.
...
Again, the confidence interval using `lm()` and `tidy()` is similar to what we saw using the bootstrap method, and thus this is available as a shortcut.
...
1. the sampling distribution of $\widehat{p}$ based on 1,000 virtual samples from the `bowl` from Subsection \@ref(shovel-1000-times) to
1. the bootstrap distribution of $\widehat{p}$ based on 1,000 virtual resamples with replacement from Ilyas and Yohan's single sample `bowl_sample_1`.
...
Here is the code to construct the bootstrap distribution of $\widehat{p}$ based on Ilyas and Yohan's original sample of 50 balls saved in `bowl_sample_1`.
...
```{r bootstrap-distribution-part-deux, fig.show='hold', fig.cap="Bootstrap distribution of proportion red for $n = 1000$.", purl=FALSE, fig.height=2.5}
...
<!-- AR: commenting this last bit out -- both because a) it's frequentist and b)
it introduces yet another way to calculate CIs but we want to focus on the
bootstrap -->
...
<!-- The formula uses the rule of thumb that 95% of values in a normal distribution are within $\pm 1.96$ standard deviations of the mean. In the case of sampling and bootstrap distributions, recall that the standard deviation has a special name: the *standard error*.  -->
...
<!-- Using these theory-based standard errors, let's present a theory-based method for constructing 95% confidence intervals that does not involve using a computer, but rather mathematical formulas. Note that this theory-based method only holds if the sampling distribution is normally shaped, so that we can use the 95% rule of thumb about normal distributions. -->
