...
<<<<<<< HEAD
<!-- EC: removed chapter references -->

=======
<!-- This is really just a test for Round 2. -->
...
<!-- EC: Include descriptions of the "more advanced" packages -->
...
<!-- EC: More chapter references need to be removed. -->
...
Recall that data on the 463 courses at UT Austin can be found in the `evals` data frame included in the **moderndive** package. However, to keep things simple, let's `select()` only the subset of the variables we'll consider in this chapter, and save this data in a new data frame called `evals_ch12`. Note that these are different from the variables chosen in Chapter \@ref(regression). 
...
Let's first look at the raw data values by either looking at `evals_ch12` using RStudio's spreadsheet viewer or by using the `glimpse()` function from the **dplyr** package:
...
if(knitr::is_html_output()){
  ggplot(evals_ch12, aes(x = age, y = score, color = gender)) +
    geom_point() +
    labs(x = "Age", y = "Teaching Score", color = "Gender") +
    geom_smooth(method = "lm", se = FALSE)
} else {
...
score_model_interaction <- lm(score ~ age * gender, data = evals_ch12)
...
evals_ch12 %>%
  rep_sample_n(size = nrow(evals_ch12), reps = 1000, replace = TRUE) %>%
  group_by(replicate) %>%
  nest() %>%
...
```{r}
lm(score ~ I(age - 30) * gender, data = evals_ch12) %>%
  tidy(conf.int = TRUE) %>%
...
gendermale_coefs <- tibble(ages = 20:90) %>%
  ...
  mutate(mod = map(ages, ~ lm(score ~ I(age - .) * gender, data = evals_ch12)),
...
par_slopes <- ggplot(evals_ch12, aes(x = age, y = score, color = gender)) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", color = "Gender") +
  geom_parallel_slopes(se = FALSE)
if(knitr::is_html_output()){
...
score_model_parallel_slopes <- lm(score ~ age + gender, data = evals_ch12)
...
interaction_plot <- ggplot(evals_ch12, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
  theme(legend.position = "none")
parallel_slopes_plot <- ggplot(evals_ch12, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_parallel_slopes(se = FALSE) +
...
newpoints <- evals_ch12 %>% 
  slice(c(1, 5)) %>% 
  augment(score_model_interaction, newdata = .)

fitted_plot <- ggplot(evals_ch12, aes(x = age, y = score, color = gender), show.legend = FALSE) +
  geom_point() +
  labs(x = "Age", y = "Teaching Score", title = "Interaction model") +
  geom_smooth(method = "lm", se = FALSE) +
