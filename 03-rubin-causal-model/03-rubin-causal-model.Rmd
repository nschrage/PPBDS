...
<!-- 1. Table centering: Stack overflow and as an issue in a Github repo. -->
...
<!-- 3. Read about estrogen case. https://www.teachepi.org/wp-content/uploads/OldTE/documents/courses/bfiles/The%20B%20Files_File1_HRT_Final_Complete.pdf -->

<!-- 4. Explain *heterogenous treatment effects*. Tie that concept to interaction terms. Discussing that now sets the stage for more concrete discussion in the regression chapters. -->

<!-- We should have a whole section devoted to bias/confounding, centered around the simple example of college attendance which I added at the end. Mention natural experiments and maybe other stuff. Highlight some examples, like the estrogen treatments which gave women cancer, where the assignment mechanism was not random. In the train example, what if we allowed the Spanish-speakers to select which platform they went on to? What if they were guided, subconsiously, to more or less crowded platforms? What if they sought to have an effect, like the perfect doctor?  -->
...
knitr::include_graphics("images/enos_seal.png")
```
...
<!-- CB: Defining estimands later in this section. Would it be worth splitting this section up? -->
...
<!-- What happens in the case that someone's control attitude is 3, and an ATE negative would reduce their expected score beyond the range of the survey? -->
...
<!-- CB: Don't know if I have explained this well? Feel like introducing interaction terms would require some introduction of regression and I don't know if that's necessary.-->
...
There will inevitably be some random variation of treatment effect for individuals in a study. When the variation in individual treatments are non-random and correlated with some factor it is described as a *heterogenous treatment effect*. For example, maybe we think that the treatment effect varies depending on the gender of an individual. In this case, two different variables for an individual would impact our estimate of treatment effect. We will learn to deal with heterogeneous treatment effects in later chapters, however it is worth keeping in mind how assumptions of homogeneity can impact our understanding of treatment effect.
...
<!-- EC: You mention second of two most common sources of missing data but the other most common source is not mentioned; you could either introduce it now or get rid of the mention. 

CB: They're both outlined below. I got rid of the last sentence so hopefully that is more clear? -->
...
<!-- DK: The above is awkward because of bad prior editing. Please fix or add to. -->
...
<!-- CB: I think this section required a different sub-heading because of me moving the missing data section. Thought of validity first. -->
...
<!-- CB: I am not completely clear on the definition of selection bias, or what its distinction is from just a very biased assignment mechanism? -->

This is an extreme example of a problem called **selection bias**.  The Spanish-speaking people are not choosing platforms to stand on randomly. Rather, they are making treatment decisions based directly on the potential outcomes of each platform.  Remember, whenever the assignment mechanism is correlated with the potential outcomes there is confounding which is a problem because it means that our estimand is biased.
...
<!-- Having trouble thinking of a necessity for non-random assignment mechanism in this example. Maybe some platforms are too crowded, so there is no way of actually knowing if anyone nearby heard the Spanish? So you have to do the experiment on a platform where the Spanish can be heard at all. --> 
...
knitr::include_graphics("animations/permutation_4.gif")
```
...
```{r, echo = FALSE, fig.fullwidth = TRUE}
knitr::include_graphics("animations/permutation_gif.gif")
```
...
knitr::include_graphics("images/survey_screenshot.png")
```
...
<!-- DK: This is important stuff, but I am not sure that I like this example. Nor am I sure that it belongs here. And it certainly merits a Rubin Table to drive the point home. -->
...
<!-- CB: Thought about putting this up with assignment mechanism too? Conflicted on where to put it. -->

```{r echo=FALSE, fig.margin = TRUE}
knitr::include_graphics("images/HRT.png")
```
