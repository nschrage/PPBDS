...
A full description of all the variables included in `evals` can be found at [openintro.org](https://www.openintro.org/stat/data/?data=evals) or by reading the associated help file (run `?evals` in the console). However, let's fully describe only the `r ncol(evals_ch11)` variables we selected in `evals_ch11`:
...
Now we have a new list column, `mod`, that contains the model objects created by `lm()`.  We can then use `map()` again to `tidy()` our model objects in order to make it easier to obtain the results:
...
We still need a column that consists of just the estimates of `bty_avg`.  We can use `map_dbl()` to obtain this:

```{r}
x <- x %>% 
  mutate(disp_coef = map_dbl(reg_results,
                             ~ filter(., term == "bty_avg") %>% pull(estimate)))
```
...
The confidence intervals reported by `lm()` and `tidy()` are very similar to our bootstrap confidence intervals.  Thus, we can interpret these confidence intervals the same way we did in Chapter \@ref(confidence-intervals).  <!-- AR: right now the confidence interval chapter appears to have a frequentist interpretation -- that should be changed. -->  In 95\% of our samples, the estimate was between the lower confidence interval bound and the higher confidence interval bound; our interpretation is that the effect of `bty_avg` on `score`, which we are trying to estimate, has a 95\% chance of falling within these bounds.^[This Bayesian interpretation of the bootstrap confidence interval is often called a [credible interval](https://en.wikipedia.org/wiki/Credible_interval) in the academic literature; you may find yourself in a statistics class that uses the phrase "confidence interval" to refer only to the [frequentist](https://en.wikipedia.org/wiki/Frequentist_inference) concept wherein the interpretation is that 95\% of the *intervals* constructed using this method in repeated samples will contain the true but unknown population parameter.  For this book, we will stick with the phrase "confidence interval" and use the Bayesian interpretation.] <!-- AR: this footnote can probably be moved to wherever confidence intervals are first discussed. -->  From now on, we'll just use `lm()` because it is simpler, but when considering how to interpret the confidence intervals, remember that you'd obtain very similar results from the bootstrap method.
...
Can we interpret these coefficients causally, using the Rubin Causal Model?  Remember the phrase *no causation without manipulation*.  If we interpreted the coefficient on Asia causally, we would be saying that a country being in Asia (rather than Africa) has a *causal effect* of 15.9 years on life expectancy: the potential outcome when "treatment" is Asia is 15.9 years higher than when "treatment" is Africa.  But does this make sense?  It's hard to imagine manipulating the continent a country is in.  Thus, it is hard to conceptualize these estimates as being the "causal effect" of being in one country versus another.
...
gov <- gov %>%
  ...
  mutate(mod = map(data, ~ lm(win_probability ~ days, data = .)),
         ...
         disp_coef = map_dbl(reg_results,
                             ~ filter(., term == "days") %>% pull(estimate)),
