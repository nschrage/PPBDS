...
In this section, we'll keep things simple for now and try to explain differences in instructor ratingserage hourly workload for that class. Could it be that instructors with lower hourly workloads also have higher ratings? Could it be instead that instructors with lower hourly workloads tend to have lower ratings? Or could it be that there is no relationship between workload and teaching evaluations? We'll answer these questions by modeling the relationship between rating and workload using *simple linear regression* \index{regression!simple linear} where we have:
...
The data on the Q Guide ratings can be found in the `qscores` data frame included in the **PPBDS.data** package. However, to keep things simple, let's `select()` only the subset of the variables we'll consider in this chapter, and save this data in a new data frame called `qscores_ch10`:

```{r}
library(PPBDS.data)

qscores_ch10 <- qscores %>%
  select(number, rating, hours, enrollment, department)
```
...
1. `department`: A categorical variable representing the name of the department in which a particular course is listed.
...
Looking at this output, we can see how the values of both variables are distributed. For example, the mean course rating was 4.18 out of 5, whereas the mean workload per week was 6.17 hours. Furthermore, the middle 50% of course ratings was between 3.9 and 4.5 (the first and third quartiles), whereas the middle 50% of hours of work falls within 3.77 to 7.6, with a maximum reported workload of 22.5 hours per week.
...
In our case, the correlation coefficient of `r cor_ch10` indicates that the relationship between overall course rating and average weekly workload in hours is "weakly negative." There is a certain amount of subjectivity in interpreting correlation coefficients, especially those that aren't close to the extreme values of -1, 0, and 1. 
...
We know that the regression line we plotted has a positive slope $b_1$ corresponding to our explanatory $x$ variable `hours`. Why? Because instructors with higher `hours` scores tend to have higher teaching evaluation `scores`. However, what is the numerical value of the slope $b_1$? What about the intercept $b_0$?  Let's not compute these two values by hand, but rather let's use a computer!
...
* `x` is the explanatory variable. In our case, `x` is set to `hours`.
* The combination of `y ~ x` is called a *model formula*. (Note the order of `y` and `x`.) In our case, the model formula is `rating ~ hours`.
...
In the `estimate` column are the intercept $b_0$ = `r evals_line[1]` and the slope $b_1$ = `r evals_line[2]` for `hours`. Thus the equation of the regression line follows:
...
The intercept $b_0$ = `r evals_line[1]` is the average teaching score $\hat{y}$ = $\widehat{\text{score}}$ for those courses where the instructor had a "beauty" score `hours` of 0. Or in graphical terms, it's where the line intersects the $y$ axis when $x$ = 0. Note, however, that while the \index{regression!equation of a line!intercept} intercept of the regression line has a mathematical interpretation, it has no *practical* interpretation here, since observing a `hours` of 0 is impossible; it is the average of six panelists' "beauty" scores ranging from 1 to 10. Furthermore, looking at the scatterplot with the regression line, no instructors had a "beauty" score anywhere near 0.
...
Of greater interest is the \index{regression!equation of a line!slope} slope $b_1$ = $b_{\text{hours}}$ of `r evals_line[2]`. The "hours" subscript indicates that this number summarizes the relationship between the teaching and average beauty score variables. Note that the sign is positive, suggesting a positive relationship between these two variables, meaning teachers with higher `hours` scores also tend to have higher teaching scores. Recall from earlier that the correlation coefficient is `r cor_ch10`. They both have the same positive sign, but have a different value. Recall further that the correlation's interpretation is the "strength of linear association". The \index{regression!interpretation of the slope} slope's interpretation is a little different:

<!-- DK: Let's make sure that these interpretations are highly consistent with Gelman and across the book. -->

> For every increase of 1 unit in `hours`, there is an *associated* increase of, *on average*, `r evals_line[2]` units of `score`.
...
Thus, our best guess of Joe's potential outcome under the "treatment" of $rating = 8$ is $4.067$.
...
x <- qscores_ch10 %>% 
  select(rating, hours) %>% 
  rep_sample_n(size = nrow(qscores_ch10), replace = TRUE, reps = 1000)

x
```

For each `replicate`, we have `r nrow(qscores_ch10)` resamples of `score` and `hours`.
...
```{r}
x <- x %>% 
  mutate(mod = map(data, ~ lm(rating ~ hours, data = .)))
...
```{r}
x <- x %>%
  mutate(disp_coef = map_dbl(reg_results, ~ filter(., term == "hours") %>% pull(estimate)))
...
<!-- What is the value $\hat{y}$ on the regression line corresponding to this instructor's `hours` "beauty" score of ??? We will mark three values corresponding to the instructor for this 21st course and give their statistical names: -->

<!-- * Circle: The *observed value* $y$ = ?? is this course's instructor's actual teaching score. -->
<!-- * Square: The *fitted value* $\hat{y}$ is the value on the regression line for $x$ = `hours` = ??. This value is computed using the intercept and slope in the previous regression table:  -->

<!-- $$\hat{y} = b_0 + b_1 \cdot x = `r evals_line[1]` + `r evals_line[2]` \cdot `r x` = `r y_hat`$$ -->

* Arrow: The length of this arrow is the *residual* \index{regression!residual} and is computed by subtracting the fitted value $\hat{y}$ from the observed value $y$. The residual can be thought of as a model's error or "lack of fit" for a particular observation.  In the case of this course's instructor, it is $y - \hat{y}$ = ??

```{r, echo=FALSE, warning=FALSE, fig.cap="Example of observed value, fitted value, and residual."}
# best_fit_plot <- ggplot(qscores_ch10, aes(x = hours, y = score)) +
...
Now say we want to compute both the fitted value $\hat{y} = b_0 + b_1 \cdot x$ and the residual $y - \hat{y}$ for *all* 463 courses in the study. Recall that each course corresponds to one of the 463 rows in the `qscores_ch10` data frame and also one of the 463 points in the regression plot.
...
regression_points <- score_model %>%
  augment() %>%
  select(rating, hours, .fitted, .resid)
regression_points
```
...
* The `hours` column represents the values of the explanatory variable $x$. This is the x-position of the `r nrow(regression_points)` black points.
* The `.fitted` column represents the fitted values $\hat{y}$. This is the corresponding value on the regression line for the `r nrow(regression_points)` $x$ values.
* The `.resid` column represents the residuals $y - \hat{y}$. This is the `r nrow(regression_points)` vertical distances between the `r nrow(regression_points)` black points and the regression line.

Just as we did for the instructor of the 21st course in the `qscores_ch10` dataset (in the first row of the table), let's repeat the calculations for the instructor of the 24th course (in the fourth row):

* `score` = 4.4 is the observed teaching `score` $y$ for this course's instructor.
* `hours` = 5.50 is the value of the explanatory variable `hours` $x$ for this course's instructor.
* `.fitted` = 4.25 = `r evals_line[1]` + `r evals_line[2]` $\cdot$ 5.50 is the fitted value $\hat{y}$ on the regression line for this course's instructor.
* `.resid` = 0.153 =  4.4 - 4.25 is the value of the residual for this instructor. In other words, the model's fitted value was off by 0.153 teaching rating units for this course's instructor.
...
<!-- score_model %>% -->
<!--   augment() %>% -->
<!--   select(score, hours, .fitted, .resid) -->
<!-- ``` -->
...
<!--   select(score, hours, .fitted, conf.low, conf.high, .resid) -->
<!-- ``` -->

<!-- Note that the uncertainty for *particular predictions* is higher than the uncertainty for our estimate of the coefficient on `hours`.  We are more confident in the *average* effect of "beauty" scores on teaching evaluations than we are on any predictions we would make for a *particular person*. -->
...
outcome, not with the predicted value for a particular hours.-->
...
Regression lines are also known as "best-fitting" lines. But what do we mean by "best"? Let's unpack the criteria that is used in regression to determine "best." Recall the plot where for an instructor with a beauty score of $x = 7.333$ we mark the *observed value* $y$ with a circle, the *fitted value* $\hat{y}$ with a square, and the *residual* $y - \hat{y}$ with an arrow. We re-display that plot in the top-left plot of the next figure in addition to three more arbitrarily chosen course instructors:
...
index <- which(qscores_ch10$hours == 2.333 & qscores_ch10$rating == 2.7)
...
index <- which(qscores_ch10$hours == 3.667 & qscores_ch10$rating == 4.4)
score_model <- lm(rating ~ hours, data = qscores_ch10)
target_point <- augment(score_model) %>%
  slice(index)
x <- target_point$hours
...
index <- which(qscores_ch10$hours == 6 & qscores_ch10$rating == 3.8)
score_model <- lm(rating ~ hours, data = qscores_ch10)
target_point <- augment(score_model) %>%
  slice(index)
x <- target_point$hours
...
```{r}
lm_score_model <- lm(rating ~ hours, data = qscores_ch10)
...
The intercept $b_0$ = `r bayes_score$coefficient[1]` is the average teaching score $\hat{y}$ = $\widehat{\text{score}}$ for those courses where the instructor had a "beauty" score `hours` of 0. Again, while the \index{regression!equation of a line!intercept} intercept of the regression line has a mathematical interpretation, it has no *practical* interpretation here, since observing a `hours` of 0 is impossible.

<!-- DK: Add discussion of "hat". Recall the p hat which we estimated last time. y hat is like that! It is not a variable that we can observe. It is an estimate. (Indeed, it is an estimate of a potential outcome!) This is different form x, which has no hat, because it is real data, something we can see. Side note: not sure if the hat versus no hat distinction works well with b_0/b_1 being things we can't see and need to estimate, but for which we do not use hat notation. -->

The slope $b_1$ = $b_{\text{hours}}$ of `r bayes_score$coefficient[2]`. The "hours" subscript indicates that this number summarizes the relationship between the teaching and average beauty score variables. The slope's interpretation is different:

<!-- DK: Let's make sure that these interpretations are highly consistent with Gelman and across the book. -->

<!-- EG: To make sure these interpretations are consistent, who should I talk to/what sections should I look at? -->

> For every increase of 1 unit in `hours`, there is an *associated* increase of, *on average*, `r bayes_score$coefficient[2]` units of `score`.
...
qscores_ch10 %>%
  ...
  geom_point(data = qscores_ch10, size = 2) +
  scale_fill_brewer() + 
...
```{r}
stan_glm(rating ~ hours, data = qscores_ch10)
```
