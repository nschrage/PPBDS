...
qscores_ch10 <- qscores %>%
  select(course_number, rating, hours, enrollment, department)
```
...
1. `course_number`: An identification variable used to distinguish between the 1 through 748 courses in the dataset.
...
The intercept $b_0$ = `r evals_line[1]` is the average teaching score $\widehat{y}$ = $\widehat{\text{score}}$ for those courses where the instructor had a "beauty" score `hours` of 0. Or in graphical terms, it's where the line intersects the $y$ axis when $x$ = 0. Note, however, that while the \index{regression!equation of a line!intercept} intercept of the regression line has a mathematical interpretation, it has no *practical* interpretation here, since observing a `hours` of 0 is impossible; it is the average of six panelists' "beauty" scores ranging from 1 to 10. Furthermore, looking at the scatterplot with the regression line, no instructors had a "beauty" score anywhere near 0.
...
<!-- What is the value $\widehat{y}$ on the regression line corresponding to this instructor's `hours` "beauty" score of ??? We will mark three values corresponding to the instructor for this 21st course and give their statistical names: -->

<!-- * Circle: The *observed value* $y$ = ?? is this course's instructor's actual teaching score. -->
<!-- * Square: The *fitted value* $\widehat{y}$ is the value on the regression line for $x$ = `hours` = ??. This value is computed using the intercept and slope in the previous regression table:  -->

<!-- $$\widehat{y} = b_0 + b_1 \cdot x = `r evals_line[1]` + `r evals_line[2]` \cdot `r x` = `r y_hat`$$ -->

* Arrow: The length of this arrow is the *residual* \index{regression!residual} and is computed by subtracting the fitted value $\widehat{y}$ from the observed value $y$. The residual can be thought of as a model's error or "lack of fit" for a particular observation.  In the case of this course's instructor, it is $y - \widehat{y}$ = ??
...
Now say we want to compute both the fitted value $\widehat{y} = b_0 + b_1 \cdot x$ and the residual $y - \widehat{y}$ for *all* 463 courses in the study. Recall that each course corresponds to one of the 463 rows in the `qscores_ch10` data frame and also one of the 463 points in the regression plot.
...
The intercept $b_0$ = `r bayes_score$coefficient[1]` is the average teaching score $\widehat{y}$ = $\widehat{\text{score}}$ for those courses where the instructor had a "beauty" score `hours` of 0. Again, while the \index{regression!equation of a line!intercept} intercept of the regression line has a mathematical interpretation, it has no *practical* interpretation here, since observing a `hours` of 0 is impossible.
