...
Similarly to the bootstrap method we just went over, this model is taking serveral resamples of the data when anayzing the distribution of heights in our data to create a posterior distribution. More specifically, it is using sampling via the Markov chain Monte Carlo. While an in depth explanation of the functions algorithm is beyond the scope of the course, you can read up on it here https://mc-stan.org/rstanarm/reference/stan_glm.html if interested.
...
Under that, we see the output for the two parameters of the model: intercept and sigma. For each parameter, ther is a calculated point estimate (medians computed through the simulation) and uncertainty estimates (standard deviations computed from the same simulations that are proportional to the median absolute deviation from the reported median)

Since we are only using one predictor, height, the intercept of the model can be interpreted as the model's best estimate of the mean height in the dataset. Recall that the function took many different resamples and found the mean of each one. The most accurate estimate of what the true mean actually is the Baysesian point estimate, the median of these means which in this case happends to be 167.5. In addition to the point estimate for the intercept, this model also provides estimates of the standard deviation of the posterior distribution (which is computed by scaling the median absolute deviation from the posterior medians). In this case, the MAD_SD for the intercept is .4, meaning that the standard deviation of the posterior distribution of mean heights is .4. As a rule of thumb, the smaller the MAD_SD, the less variation ther are among mean heights in the samples generated, and thus the more robust our estimate is 

The second parameter, sigma, will be calculated for all bayesian models you compute (no matter how many predictors) and shows us the average error of  model, the deviation between the predicted value and the observed value. More specifically, sigma, the residual standard error, tells us that the mean height of males will be between plus or minus 21.8 of our prediction 68% of the time and plus or minus two times 21.8 the prediction about 95% of the time. Similar to with the intercept, the MAD_SD of the sigma reflects the standard deviation of the sigma value itself (since so many different samples were taken with each its own sigma)


<!-- Show model output on nice table (maybe stargazer) -->
