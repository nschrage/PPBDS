...
# Just calculating some values here that I need for the bootstrap distributions
# later on. Will not display to student. This code should be cleaned up to
# produce just two tibbles with the needed values which can then be grabbed
# later.

mean_income_value <- trains %>% 
  pull(income) %>% 
  mean()

mean_income_value_democrat <- trains %>% 
  filter(party == "Democrat") %>% 
  pull(income) %>% 
  mean()

mean_income_value_republican <- trains %>% 
  filter(party == "Republican") %>% 
  pull(income) %>% 
  mean()

median_income_value <- trains %>% 
  pull(income) %>% 
  median()

median_income_value_democrat <- trains %>% 
  filter(party == "Democrat") %>% 
  pull(income) %>% 
  median()

median_income_value_republican <- trains %>% 
  filter(party == "Republican") %>% 
  pull(income) %>% 
  median()
...
Let's review the material from  Chapter \@ref(one-parameter) by calculating the confidence interval for 90th percentile of the distribution of age for Boston commuters.

<!-- I should either add a bunch of comments to the R code chunk, or create a margin note. Or maybe this should just be using the mean? -->

```{r}
trains %>% 
  select(age) %>% 
  bootstraps(1000) %>% 
  mutate(age_90th = map_dbl(splits, ~ analysis(.) %>% 
                                      pull(age) %>% 
                                      quantile(prob = c(0.9)))) %>% 
  ggplot(aes(x = age_90th)) +
...
quantiles <- x %>% 
  pull(mean_income) %>% 
  quantile(c(.025, .975), names = FALSE)
