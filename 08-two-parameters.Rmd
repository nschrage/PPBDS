...
`skim()` shows us what the different values of `treatment` are because it is a factor. Unfortunately, it does not do the same for character variables like `party`. The ranges for `age` and `att_end` seem reasonable. Recall that particpants were asked three questions about immigration issues, each of which allowed for an answer which was a strength of agreement between 1 and 5, with higher values indicating more agreement with the conservative position. So, the most liberal possible answer was 3 and the most conservative as 15.



## Model `age` as a function of `party`
...
We could simplify this code by combining all the `mutate()` steps into a single line, create one function which does the `analyst()`, `pull()`, `mean()` and `quantile()` steps together. However, when building up code like this, we prefer to go line-by-line, at least until everything is working.
...
x %>% 
  ggplot() +
    ...
    geom_histogram(aes(x = age_90th), binwidth = .1, fill = "red") +
    labs(x = "90th Percentile of Age",
        title = "Posterior Distribution of the 90th Percentile 
                 of Age for Boston Commuters")
```

If we were interested in the mean age, and our associated uncertainty, we could use this exact code, except replacing `quantile()` with `mean()`. Yet the mathematical convenience of the mean allows us to avoid the complexity of the bootstrap.


But what if, instead of having no other information about our sample beyond their ages, we also knew their political party? How would we make a model which included this other information?

### Two Parameters

Let's begin by using `lm()`, in almost the same was as we did in Chapter \@ref(one-parameter).
