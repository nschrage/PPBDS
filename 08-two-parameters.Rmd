...
<!-- DK: Should also add a "validity" or "representativeness" discussion about what population this estimate applies to. All people? All people in Boston? All Boston commuters? -->
...
Let's begin by using `lm()`, in almost the exact same was as we did in Chapter \@ref(one-parameter). Start by estimating a simple model:
...
The initials "lm" stand for "linear model." We are creating a model in which age is a function of party. The "-1" term causes the model to be estimated without an intercept, a concept which will be explained in Chapter \@ref(continuous-i). The coefficient estimates are the same as the mean values for age within each party.
