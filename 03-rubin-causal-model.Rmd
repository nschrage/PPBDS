...
<!-- 0) Read chapter 5. And set up PPBDS.data. Submit pull request with a tutorial.Rmd for your chapter with a few questions in it.CB: Done -->

<!-- 1) Zoom with Tahmid. Add Preceptor Table discussion to chapter 7, in a couple of places. The only things that belong in a Preceptor Table are numbers, or posterior distributions of those numbers. -->

<!-- Set up Zoom with Tahmid. Preceptor Table. Ton of subtle issues. First, wish we had everyone's height. Then, look up is easy. But you don't have that, so you take the 10,000 data you do have, fill them in, and then do statistics. Second is the issue of age and gender and other stuff you might also know. Once I tell you it is an adult, then we would have a different Preceptor Table. And then we fill in just the adult data from nhanes, not all the data. Lesson: Sometimes people give you data which is irrelevant to the task at hand. There is a different Preceptor Table for every problem.

Go back to basic case. Key is coming up with a model and filling in the question marks. RT has 7 billion rows. But all the missing ones are the same! So, really, just look at 10,000 + 1 row. Quetion: What do we replace that one question mark with? Depends on the question we are asking! One answer: best guess. Another: 95% confidence interval for best guess. Another: 95% confidence interval for forecast of actual value. These three things correspond to three key themes: fitted value, parameter uncertainty, prediction interval. -->


<!-- In many ways, everything starts with the Preceptor Table. There is no *true* Preceptor Table. (Or, perhaps, the true Preceptor Table is infinite in all directions.) There is only the Preceptor Table which is most useful for the task we have. We start with our problem --- the decision we must make --- and then go from there. Example: If you're job is to simply predict a new person's attitude toward immigration, then you don't care about causation. There is only one Y column: att_end for this person. There is not both a Y_t and a Y_c because this person has already either been treated or not. Whichever it is, you don't care about what it would have been in some other counterfactual world. So, you make a Preceptor Table which is very different looking from the Preceptor Table which someone who cares about the causal effect for this person would make. -->

<!-- In fact, the world is even more complex than this! There are two extreme cases. With prediction, there is only one Y column. With "perfect" causal inference (in the simplest case with one treatment and one control), there are two Y columns, Y_c and Y_t. But most real-world situations fall somewhere between these two ideals. For example, in the governor example discussed below, we want to determine the effect of winning an election on life length. Do people who win elections live longer than people who don't? This seems like a simple causal inference situation in which Y is length of life and we want to compare Y_t (if you win election) against Y_c (if you don't). But, for many people Y_t is, by definition, impossible. Consider Joe, who is just 20 years old. In his state, the age minimum for being Governor is 35. So, it is impossible to even imagine that we could observe Y_t.  -->

<!-- To show this, we need a new Preceptor Table in which, instead of rows in which Y always have two columns, we have a Y which mostly has two columns but sometimes only has 1. What's nice about this example is that it generalizes to other cases. -->
...
<!-- Fifth, connect all this to the central theme of 98% of social science research --- using observational data to infer causation by treating the assignment mechanism as being close enough to random. This is where we can discuss stuff like natural experiments, difference in differences, regression discontinuity and so on. Let's discuss. (Save this for next week.) -->

<!-- DK: Consider the governors paper from PPBDS.data. This is a great, concrete example which might serve as the second key data set in the chapter. It provides a structure to discuss all the issues which make it hard --- but sometimes still possible! --- to use observational data to estimate causal effects. Let's outline some of the things we might discuss: -->
...
<!-- CB: Do we need to be capitalizing the adjectives before Preceptor Table every time? Ie. Ideal, Infinite and Actual? -->
...
<!-- CB: Should I be explaining how to find average treatment effect in this section? No! Because it will be the same as the average treatment effect later on with the only difference being that we are emphasizing in the later section that we aren't trying to understand individuals and fill in the table but rather just want to know the average more generally. But I also feel like it is necessary to set up the two parameters section. --> 
...
<!-- SJ: In need of a concrete definition of "Fundamental Problem of Casual Inference", I know you have described it above but maybe make it more clear explain it more and put it in bold or something? The reader should have no doubt in their mind that they read about it before when you mention it now. CB: Think this is the most common thing about all of Shea's comments that the fundamental problem is not clear enough. -->
...
<!-- CB: Again, does this section make sense given that we can't manipulate age/sex?  -->
