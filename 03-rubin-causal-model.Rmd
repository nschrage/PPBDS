...
<!-- No bolding ID column? ID should be left justified. CB: Done-->
...
$Y_{t}-Y_{c}$ is the causal effect of there being Spanish speakers on the platform. Yao would have an attitude score of $13$ if there were Spanish speakers and $9$ if there were not; the causal effect of the presence of Spanish speakers for Yao is is $4$.
...
We now have two different estimates for Emma. When we estimate her Attitude if Control using an assumption of constant treatment effect (a single value for tau), we estimate $Y_c(Emma)$ to be 12.33. When we estimate assuming treatment effect is constant across sexes, we get $Y_c(Emma)$ to be 10. This difference between our estimates for Emma is the heart of inference. 
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
  ...
  cols_align(align = "center", columns = TRUE) %>%
  cols_align(align = "left", columns = vars(subject)) %>%
  tab_spanner(label = "$$Outcomes$$", vars(ytreat, ytreat2, ycontrol)) %>%
  fmt_markdown(columns = TRUE)
```
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
  ...
  cols_align(align = "center", columns = TRUE) %>%
  cols_align(align = "left", columns = vars(subject)) %>%
  tab_spanner(label = "$$Outcomes$$", vars(ytreat, ytreat2, ytreat3, ytreat4, ytreat5, ycontrol)) %>%
  fmt_markdown(columns = TRUE)
```
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
  ...
  cols_align(align = "center", columns = TRUE) %>%
  cols_align(align = "left", columns = vars(subject)) %>%
  tab_spanner(label = "$$Outcomes$$", vars(ytreat, ytreat1001, ytreat1002, yellipsis, ycontrol)) %>%
  fmt_markdown(columns = TRUE)
```
...
tibble(subject = c("Person 1", "Person 2", "Person 3", "...", "Person N"),
  ...
  cols_align(align = "center", columns = TRUE) %>%
  cols_align(align = "left", columns = vars(subject)) %>%
  tab_spanner(label = "Outcomes", vars(`Attitude`))
```
