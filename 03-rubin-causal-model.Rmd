...
<!-- Preamble: George Bailey. Show me a movie. Or a gif. End with RCM paragraph. CB: Done -->

<!-- 1) Preceptor Table. Formal definition. Harvard example with misssing rows. Promise to get more technical later. CB: done -->

<!-- 2) Causal effect. Three header columns: outcomes, covariates, estimands CB: Done -->

<!-- 3) Simple models. One parameter. Two parameter. CB: done, just change section title-->

<!-- 3) Introduce the themes! Prudence: Infinite Preceptor Table (Is Yao the same? Are the treatments the same? Are new people and new treatments similar enough? Is this treatment effect connected to what my real problem? I don't really care how to change how people fill out anonymous surveys.) CB: Done Justice: math. Courage: Write some code. Temperance (representativeness). -->
...
`r knitr::include_app("https://www.youtube.com/embed/cR7p-IB6INM", height = "400px")`
...
### Introduction to the Preceptor Table
...
Now we can look at a very simple statistical model. Say you have 5 adult brothers and you are given 4 of their heights. What is the 5th brother's height? Because we are not given it, we can use statistics to make a best guess. Let's look at a Preceptor Table for our problem:
...
gt() %>%
  cols_label(subject = md("$$\\mathbf{ID}$$"),
                ytreat = md("$$Y_t$$"),
                ycontrol = md("$$Y_c$$")) %>%
  tab_style(cell_borders(sides = "right"),
            location = cells_body(columns = vars(subject))) %>%
...
In this chapter we don't have very much code, as a lot of what we have discussed is very conceptual. Permutation tests however are an example of a time when we need courage to create code that will allow us to better understand the accuracy of our model. 
...
You will need a lot of courage when learning about these more complex models in future chapters!
