...
<!-- 0) Read the themes.Rmd about predictive versus causal models. Consider the issue of one column Y versus two column Y. Think about where the missing data section belongs. Edit accordingly. CB: Done -->
...
<!-- But none of that is causal! It is only predictive. CB: Done. -->

<!-- 1) Two sources of difficulty. First, some rows have only one column. This is really confusing. What is the meaning of average causal effect in that world? Can't talk about average causal effect for whole population. Need a Preceptor Table with fewer rows. Can only estimate the average causal effect for the population of people who are allowed to be governor. How do we estimate it? Related issue is: What causal effect do we really care about? "What is the causal effect on the treated?" CB: Done -->

<!-- Example: Does Enos allow us to calculate a causal effect for people who commute by cars? Can we calculate the causal effect for people in New York City? Barcelona? Representativeness --- add a paragrpah about this. Do the experimental estimates apply outside the experiment. CB: Done -->
...
<!-- CB: Should this be a new section or in the preamble? Doesn't really fit with 'What is a causal effect?', but could make it a subheading in that section. -->

Let's start with a very simple statistical model. Say you have 5 adult brothers and you are given 4 of their heights. What is the 5th brother's height? Because we are not given it, we can use statistics to make a best guess. Let's look visually at our problem:

`r margin_note("This is an example of what we will call a Preceptor Table. We will go more into depth about what this means soon, but keep that term in mind.")`
```{r echo = FALSE}
tibble(Brothers = c("Robert", "Nicholas", "Beau", "Ishan", "Andy"),
       Heights = c("178", "165", "172", "173", "?")) %>%
  
  # Then, we use the gt function to make it pretty
  
  gt() %>%
  cols_label(Brothers = md("**Brothers**"),
                Heights = "Heights (cm)") %>%
  tab_style(cell_borders(sides = "right"),
            location = cells_body(columns = vars(Brothers))) %>%
...
As you can already begin to see, we should consider even the most simple statistical models with some care. Now let's think about another problem we might want to know. Say we have the heights of 100 Harvard students, and from that we want to know the average height of students in the school. Again, are these 100 students randomly sampled? Could we estimate the 90th percentile of height in the student population? These questions are more complicated, and we might be less confident in our best guesses. Now let's say we are given some characteristics other than height for every student at Harvard, like sex and age. Would we be able to forecast a random students height? 
...
So far, we have only asked predictive questions! (Can we forecast or predict the fifth brother's height?) This chapter primarily focuses on causal inference. In the case of a causal model, we want to understand the effect of some treatment. Let's start with an example experiment about Boston commuters on a train platform. 
...
`r margin_note("Don't be scared by the fancy term Preceptor Table: you have already seen some in this chapter! What is important is understanding the structure of a Preceptor Table so they can help you to model and answer the questions you have.")`
...
<!-- CB: From Miro> Needs to focus less on parameter uncertainty and more on the perfect doctor idea -- you never know if you are assigning the treatments to a truly random selection of people CB: new section idea for when randomization fails/doesn't work -->

<!-- CB: Do not like how this section now goes up to 3.4.12 but that might just be personal preference -- Is there a better way to break it up?  -->
...
<!-- CB: Themes.Rmd, knowing even if we have a good model, there are still things we can't model that precisely.. goes nicely into mathmatical models section.  -->
...
<!-- CB: This needs to be potentially moved higher up in the chapter. -->

Models help us to infer things in both prediction and causal inference. This chapter is all about the Rubin Causal Model, so naturally we have been focused on causal models. We should, however, further discuss the subtleties that differentiate predictive and causal models. 
...
<!-- CB: causal effect on the treated is all we care about for governors paper, discuss here. -->
...
This brings up an crucial point of the importance of choosing appropriate estimands. This is an important concept in real world experiments, like testing the effectiveness of a certain drug. Two estimands someone might be interested are the effectiveness of the drug as actually taken (de facto) and the effectiveness of the drug if taken as directed (de jure). ^[Mallinckrodt et al. 2017] Choices in estimand could impact big desisions like if a drug is approved or not. 

Although in this chapter we have discussed lots of concepts at the surface level, it is important to keep in mind how these same questions and assumptions can have serious implications in the real world.

<!-- CB: https://www-ncbi-nlm-nih-gov.ezp-prod1.hul.harvard.edu/pmc/articles/PMC1874399/ -->

<!-- CB: Transition sentence/paragraph between these sections tie in the idea of social science/observations -->
