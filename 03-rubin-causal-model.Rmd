...
<!-- 1) Zoom with Tahmid. Add Rubin Table discussion to chapter 7, in a couple of places. The only things that belong in a Rubin Table are numbers, or posterior distributions of those numbers. -->

<!-- Set up Zoom with Tahmid. Rubin Table. Ton of subtle issues. First, wish we had everyone's height. Then, look up is easy. But you don't have that, so you take the 10,000 data you do have, fill them in, and then do statistics. Second is the issue of age and gender and other stuff you might also know. Once I tell you it is an adult, then we would have a different Rubin Table. And then we fill in just the adult data from nhanes, not all the data. Lesson: Sometimes people give you data which is irrelevant to the task at hand. There is a different Rubin Table for every problem.
...
<!-- In many ways, everything starts with the Rubin Table. There is no *true* Rubin Table. (Or, perhaps, the true Rubin Table is infinite in all directions.) There is only the Rubin Table which is most useful for the task we have. We start with our problem --- the decision we must make --- and then go from there. Example: If you're job is to simply predict a new person's attitude toward immigration, then you don't care about causation. There is only one Y column: att_end for this person. There is not both a Y_t and a Y_c because this person has already either been treated or not. Whichever it is, you don't care about what it would have been in some other counterfactual world. So, you make a Rubin Table which is very different looking from the Rubin Table which someone who cares about the causal effect for this person would make. -->
...
<!-- To show this, we need a new Rubin Table in which, instead of rows in which Y always have two columns, we have a Y which mostly has two columns but sometimes only has 1. What's nice about this example is that it generalizes to other cases. -->
...
In order to get an idea of an Estimand, let's look again at our basic Rubin Table for Yao:
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Thamid", "Diego"),
...
- The most negative causal effect.  Here, that is $-3$, from Thamid ($9 - 12 = -3$).
...
Remember, the data we actually observe looks something like this:
...
Question marks are responses that could not be observed. The Fundamental Problem of Causal Inference^[Holland 1986.] is that directly observing unit-level causal effects is impossible. If we don't have all the values in our table, how do we calculate anything?
...
Remember what our Rubin Table looks like with all of the missing data: 
...
3. As we did earlier, if you are willing to assume that the causal effect is the same for everyone (a big assumption!), you can use your estimate of the ATE to fill in the missing individual values in your Rubin Table.
...
So estimating the ATE is easy ($Y_c(u) = Y_t(u)$).  But is our $\widehat{ATE}$ a good estimate of the actual ATE?  After all, if we knew all the missing values in the Rubin Table, we could calculate the ATE perfectly.  But those missing values may be wildly different from the observed values.  Consider this unobservable true Rubin Table:
...
Let's start by looking at what kinds of missing data make up the infinite Rubin Table. For example, say we only care about the causal effect of this experiment on Yao. Do we only care about his attitude right after the experiment? No! We also care about Yao's potential outcomes one year from now, two years from now, and so on.

<!-- SJ: Also, it might seem obvious to you while writing this, but maybe explain why we should care about Yao's attitude a year or two from now. This will bring more utility to the assumptions we are making and a nice clear real world connection to why we are studying this causal model -->

So our full Rubin Table includes people we know (Yao) and people we don't (for example, Eliot), both now and in the future:
...
In fact, because time is continuous, there is a row for Yao now, Yao one second from now, Yao one day from now and so on. The Rubin Table extends downward forever. Thus, in order to estimate any causal effect, we need *assumptions*, so we aren't dealing with an infinite table. 
...
Now, let's consider our Rubin Table for Yao, Emma, Cassidy, Thamid, and Diego again:
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Thamid"),
       ytreat = c("13", "11", "?", "?"),
       ycontrol = c("?", "?", "10", "12"),
       ydiff = c("?", "?", "?", "?")) %>%
...
Let's see what this would look like in a table similar to a Rubin Table:
