...
math_notation <- tibble(subject = "Yao",
       ytreat = "13",
       ycontrol = "9",
       ydiff = "+4") %>%
...
<!-- DK: Why isn't this a reasonable estimate? Because it might be below the lowest possible value of 3? Well, maybe? I think we might just cut this out. -->
...
We would have two different estimates for $\tau$. $\tau_M$ would be $$(13+5)/2 - 12 = -3$$. $\tau_F$ would be $$(11-10 = +1)$$. Using those values, we would fill out our new table like this: 
...
Another example of a *heterogeneous treatment effect* is when the variation in individual treatments are non-random and correlated with some or multiple factors. For example, maybe we think that the treatment effect varies depending on the sex and age of an individual. In this case, sex, age and treatment status are three variables which all have impact on the treatment effect. The three variables might interact, influencing the treatment effect across these characteristics. In other words, the interaction effect of these three variables gives us a heterogeneous treatment effect. 
...
<!-- DK: This transition is a bit awkward. And it needs to be brought back to assumptions we make. That is, the true Rubin Table has an infinite number of rows, even if all we care about is Yao! But such a reality is unworkable. So, we make assumptions! We *assume* that the causal effect for Yao is the same a minute from now as it is now, and a day from now, and . . . probably not 30 years from, now, but for some period of time. So, we can reduce the true problem to the problem we were dealing with before. -->
