...
<!-- Post question about getting rid of italicized ID in math notation table. CB: Didn't need to. -->

<!-- Ugly ydiff in table column CB: Done -->

<!-- Get estimands versus estimand and other terms precise. CB: Done -->

<!-- Put hetergemous treatment effects before ATE section. Include full table with \tau_{Yao}. -->

<!-- Can I still make progress if I am unwilling to assume a single tau? yes! -->

<!-- Keep or throw away permutation videos. -->

<!-- Throw away permutation tests. CB: Done. -->

 <!-- Fix the tutorial. Delete the True Agains. Give enough blank lines for what you  -->

<!-- Rubin quotes in side margin. CB: Done-->

<!-- Left justify ID in tables. CB: Done but want to see if I can change text size of everything and fix italicized estimands and outcomes.  -->
...
By showing what the world would have been like without George, we get an idea of the causal effect of his life on the people he interacted with. We will be looking at causal effects throughout the rest of this chapter using the Rubin Causal Model. 
...
In this case, we have a row for each brother and a column for their height. We will always have an ID column in Preceptor Tables in which we can identify different units. `r margin_note("The ID column is always furthest to the left.")` In addition to the ID column, we call the column with the brother's heights an outcome column. `r margin_note("Outcomes columns are always right after the ID column.")` There are other types of columns that might be included in a Preceptor Table that we will introduce soon!
...
One possible variable we might be interested is causal effect. `r margin_note("Remember this is the difference between his potential outcomes under treatment and control.")` Let's add this to our ideal Preceptor Table:
...
But remember that in a actual Preceptor Table we will have a bunch of missing data! We normally can't use simple arithmetic to calculate the causal effect on Yao. Instead we will be required to estimate. An **estimand** is some variable in the real world that we are trying to measure. `r margin_note("The estimand in this case is $Y_{t}-Y_{c}$, not $+4$.")` An estimand is not the number value calculated, but rather the variable you want to understand from your data. 
...
Treatment effect is only one possible estimand we might be interested in. The ratio of potential outcomes is another example:
...
tibble(subject = "Yao",
  ...
  fmt_markdown(columns = TRUE) %>%
  tab_spanner(label = "$$Outcomes$$", vars(ytreat, ycontrol)) %>%
  tab_spanner(label = "$$Estimand$$", vars(yperc))
```

Generally a study has individuals who each have their own unique potential outcomes. In that case, it no longer makes sense to just have $Y_t$ and $Y_c$ represent all of the individual potential outcomes. A new variable is needed to differentiate between individuals. 

In other words, there needs to be a distinction between $Y_t$ for Yao, and $Y_t$ for Emma. We could do something like $Y_t(Yao)$ and $Y_t(Emma)$, but that would be an ineffective approach if we wanted to fill out a full Preceptor Table. Instead, we use the variable $u$ ($u$ for units) to indicate that the treatment effect varies by each individual unit (person) in a study. The $u$ represents all the different individuals in a study without requiring that we create a new variable for each person. 

Now, instead of $Y_t$, let's use $Y_t(u)$ to represent 'Attitude if Treated'. If you want to talk about only Emma, you could say "Emma's Attitude if Treated" or "$Y_t(Emma)$" or "the $Y_t(u)$ for Emma", but not just $Y_t$. That notation is too ambiguous when there is more than one subject you might be talking about.
...
From this ideal Preceptor Table, there are many possible estimands we might be interested in. Here is just a sampling:
...
In the train study, all of the individuals surveyed are different in many different ways, for example gender, income, and age. Therefore, our study population is heterogeneous. When we estimate ATE, we are making assumptions that there is a similar treatment effect for all subjects in the study, despite the heterogeneity of our sample.
...
Let's assume there is a heterogenous treatment effect for every individual in our study. Our actual Preceptor Table would therefore look something like this: 

```{r, echo = FALSE}
# First, we create a tibble with the values we want for the table

tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
       ytreat = c("13", "11", "$$10 + \\tau_\text{Cassidy}$$", "$$12 + \\tau_\text{Tahmid}$$", "5"),
       ycontrol = c("$$13 - \\tau_\text{Yao}$$", "$$11 - \\tau_\text{Emma}$$", "10", "12", "$$5 - \\tau_\text{Diego}$$"),
       ydiff = c("$$\\tau_\text{Yao}$$", "$$\\tau_\text{Emma}$$", "$$\\tau_\text{Cassidy}$$", "$$\\tau_\text{Tahmid}$$", "$$\\tau_\text{Diego}$$")) %>%
...
Can we solve for just $\\tau_\text{Yao}$. No! So how can I make any progress from here if I am unwilling to assume there is a single or two values for tau? 
...
The average treatment effect is the average difference in *potential* outcomes between the treated group and the control group. The difference between this estimand and estimands like $\tau$, $\tau_1$ and $\tau_2$, is that in this case we do not care about using the average treatment effect to fill in missing values in each row. The average treatment effect is useful because we don't have to assume anything about each individuals' $\tau$, like $\\tau_\text{Yao}$, but can still understand something about causal effect. 
...
Let's take a step back and say we can't assume across genders for estimates of $\tau_1$ and $\tau_2$. Say we also can't assume that treatment effect is constant for everyone. What are we supposed to do now that we can't make either of those assumptions? This is where $\widehat{ATE}$ is helpful. 
...
So estimating the ATE is easy ($Y_c(u) = Y_t(u)$).  But is our $\widehat{ATE}$ a good estimate of the actual ATE?  After all, if we knew all the missing values in the Preceptor Table, we could calculate the ATE perfectly.  But those missing values may be wildly different from the observed values.  Consider this unobservable ideal Preceptor Table:
...
In this example, there is indeed a constant treatment effect for everyone: $+2$. Note that the *observed* values are all the same, but the unobserved values were such that our estimated ATE, $-1.33$, is pretty far from the actual ATE, $+2$. `r margin_note("The best way, as we'll see, is randomized assignment to treatment.")` We'll consider in a little bit when it is reasonable to assume that our estimate of the ATE is a good one. If we think we have a reasonable estimate of ATE, using that value as a constant for $\tau$ might be our *best guess* if we do want to estimate an unobserved potential outcome for an individual. 
...
Fortitude is synonymous for courage, forbearance, strength and endurance. The ability to confront fear, uncertainty and intimidation.

```{r echo=FALSE, fig.margin=TRUE}
knitr::include_graphics("other/images/fortitude.jpg")
```

The three languages of data science are words, math and code, and the most important of these is code. We need to explain the structure of our model using all three languages. Fortitude is important in helping us to create the *data generating mechanism* via code. 
