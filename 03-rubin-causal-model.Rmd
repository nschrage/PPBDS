...
<!-- SJ: Confused here, it seems it is assumed we know why it is impossible to see both outcomes at once? as a reader I don't know why that is maybe a little more explanation there? CB: Emailed with Shea, going to add a section about how we cannot assume homogeneity later in the chapter? Or in this section? and reference it here so that if people are confused they can reference that? -->
...
<!-- CB: Should I be explaining how to find average treatment effect in this section? Because it will be the same as the average treatment effect later on with the only difference being that we are emphasizing in the later section that we aren't trying to understand individuals and fill in the table but rather just want to know the average more generally. But I also feel like it is necessary to set up the two parameters section. --> 
...
We now have two different estimates for Emma. When we estimate her Attitude if Control using an assumption of constant treatment effect, we estimate $Y_c(Emma)$ to be 12.33. When we estimate assuming treatment effect is constant across sexes, we get $Y_c(Emma)$ to be 10. This difference between our estimates for Emma is the heart of inference. <!-- CB: I am confused now because thinking about no causation without manipulation. Is it possible to say that gender does not "cause" the effect, but rather treatment effect just varies by gender? Or no? -->
...
<!-- CB: This needs to go somewhere later -- right before confounding  -->
...
<!-- SJ: It could be helpful to add a note saying, "In other words, the louder/quieter the Spanish is being spoken on the train likely influences the outcome." This may seem obvious but it is just very helpful to be constantly reminded the larger picture of the choices we make as data scientists and reasons for creating tibbles with different treatments CB: Done?   -->
