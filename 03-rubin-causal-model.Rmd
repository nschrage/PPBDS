...
<!-- Fix weird gt formatting, ?gt_latex_dependencies -->
...
`r margin_note("A parameter is some variable we can calculate using our data. A parameter can be an estimand by itself, like average of the control group. An estimand can also be some more complex calculation involving multiple parameters.")`
...
We now have two different estimates for Emma. When we estimate $Y_c(Emma)$ using an assumption of constant treatment effect (a single value for tau), we get 12.33. When we estimate assuming treatment effect is constant across sexes, we get $Y_c(Emma)$ to be 10. This difference between our estimates for Emma is the heart of inference. 
...
This assumption is unfortunately rarely true. There will inevitably be some random variation of treatment effect for individuals. The effect on Yao may be different than the treatment effect on another individual that is seemingly very similar to Yao. This is one example of a *heterogeneous treatment effect*. 
...
Let's assume there is a heterogeneous treatment effect for every individual in our study. Our actual Preceptor Table would therefore look something like this: 
...
Can we solve for just $\tau_{yao}$. No! That is the Fundamental Problem of Causal Inference. So how can I make any progress from here if I am unwilling to assume there is a single or two values we can estimate for tau? 
...
If we already did this exact same calculation above, why are we talking about it again? Remember that we unwilling to assume treatment effect is constant in our study population, and we cannot solve $\tau$ for individuals. This is where $\widehat{ATE}$ is helpful. 
...
Instead of considering the treatment in terms of duration, we could also consider different volume levels at which Spanish is being spoken. We might also think that the volume at which the Spanish is being spoken on the train platform could influence potential outcomes.
...
Prudence must be used in order to avoid becoming overwhelmed with the real world, and instead to find solutions that allow us to begin answering some of our questions. 
...
`r margin_note("Think about how the cardinal virtues can be applied to your understanding of these other issues with causal inference.")`
