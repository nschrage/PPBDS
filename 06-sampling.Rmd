...
<!-- Discuss hypothesis tests and why we hate them. Should do this in every chapter through the core of the book. -->
...
## Sampling urn activity {#sampling-activity}
...
What we just demonstrated in this activity is the statistical concept of \index{sampling} *sampling*. We would like to know the proportion of the urn's balls that are red. Because the urn has a large number of balls, performing an exhaustive count of the red and white balls would be time-consuming. We thus extracted a *sample* of 50 balls using the shovel to make an *estimate*. Using this sample of 50 balls, we estimated the proportion of the *urn's* balls that are red to be 34%.
...
if(!file.exists("data/virtual_samples_25.rds")){
  virtual_samples_25 <- urn %>% 
    rep_sample_n(size = 25, reps = 1000)
  write_rds(virtual_samples_25, "data/virtual_samples_25.rds")
} else {
  virtual_samples_25 <- read_rds("data/virtual_samples_25.rds")
}
virtual_prop_red_25 <- virtual_samples_25 %>% 
  group_by(replicate) %>% 
  summarize(red = sum(color == "red")) %>% 
...
if(!file.exists("data/virtual_samples_50.rds")){
  virtual_samples_50 <- urn %>% 
    rep_sample_n(size = 50, reps = 1000)
  write_rds(virtual_samples_50, "data/virtual_samples_50.rds")
} else {
  virtual_samples_50 <- read_rds("data/virtual_samples_50.rds")
}
virtual_prop_red_50 <- virtual_samples_50 %>% 
  group_by(replicate) %>% 
  summarize(red = sum(color == "red")) %>% 
...
if(!file.exists("data/virtual_samples_100.rds")){
  virtual_samples_100 <- urn %>% 
    rep_sample_n(size = 100, reps = 1000)
  write_rds(virtual_samples_100, "data/virtual_samples_100.rds")
} else {
  virtual_samples_100 <- read_rds("data/virtual_samples_100.rds")
}
virtual_prop_red_100 <- virtual_samples_100 %>% 
  group_by(replicate) %>% 
  summarize(red = sum(color == "red")) %>% 
