...
```{r sampling-exercise-2, echo=FALSE, fig.cap="Inserting a shovel into the urn.", purl=FALSE, out.width = "100%"}
knitr::include_graphics("06-sampling/images/sampling_bowl_2.jpg")
```


```{r sampling-exercise-3, echo=FALSE, fig.cap="Removing 50 balls from the urn.", purl=FALSE, out.width = "100%"}
...
```
```{r samplingdistribution-virtual-1000, echo=FALSE, fig.cap="Distribution of 1,000 proportions based on 1,000 samples of size 50."}
virtual_prop_red <- virtual_samples %>% 
  group_by(replicate) %>% 
  summarize(red = sum(color == "red")) %>% 
  mutate(prop_red = red / 50)
...
<!-- DRAFT: Let's apply these ideas to gambling. If we were to bet money on the proportion of the red balls in the urn, and we could perform virtual simulations beforehand, we would want the most *accurate* and most *precise* estimate. Here, our *estimand*, the thing we want to know, would be the proportion of red balls in the urn. INCORP DISCUSSION OF PREDICTION UNCERTAINTY, UNMODELED VARIATION, PARAMETER UNCERTAINTY,  -->

<!-- MF: Stick with urn example for gambling? Something less predictable & involving more unknowns might be better for explanining model structure, paramater uncertainty, prediction uncertainty, etc. On the other hand, I don't want this chapter to get too lengthy.  -->
...
Now that we have mastered sampling, let's consider how these scenarios connect to the Rubin Causal Model (RCM). For the most part, they don't! 

Remember the RCM's slogan: *No causation without manipulation*. In both the urn example and our Obama poll, we are not manipulating our units (the balls and the opinions of the pollees, respectively). There are not a control and treatment condition to compare; there is no causal effect to measure. That must mean that the RCM is completely irrelevant, right?

Not exactly. Certain elements of the RCM *can* be applied here. 

In both the RCM and in our exercises, randomization is *incredibly important* to ensure validity and precision. In the RCM, this randomization exists in the form of  **random assignment**, the act of assigning units a condition (control or treatment) randomly. In sampling, this randomization exists in the form of **random sampling**.

<!-- MF: Still wondering how to incorporate RCM into this chapter, since we are not dealing with assignment & two realities (one observed, one not observed) for each ball. Should this section focus on proving that RCM is still relevant, or explaining that RCM can't be applied perfectly to the urn scenario? -->
