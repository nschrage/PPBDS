...
<!-- MF: Need clarification on joint distribution after speaking with Vivian. Only every 100 beads? May have to reach out to Ishan instead, since he created that distribution. --> 
...
<!-- Need to incorporate some notion of betting. The over/under line for percentage of red beads in the bowl. Can you show (maybe a problem set?) that the mean is the best place to put the over/under line? Play the prediction game.  -->
...
Take a look at the urn in Figure 6.1. It has a certain number of red and a certain number of white beads all of equal size. It appears the urn has been mixed beforehand, as there does not seem to be any coherent pattern to the spatial distribution of the red and white beads.
...
One way to answer this question would be to perform an exhaustive count: remove each bead individually, count the number of red beads and the number of white beads, and divide the number of red beads by the total number of beads. However, this would be a long and tedious process. 

### Using the shovel method once 

Instead of performing an exhaustive count, let's insert a shovel into the urn as seen in Figure 6.2. Using the shovel, let's remove $5 \cdot 10 = 50$ beads, as seen in Figure 6.3. Here, we are taking a *random sample* of the total population of beads. 
...
However, say, we started this activity over from the beginning. In other words, we replace the 50 beads back into the urn and start over. Would we remove exactly 17 red beads again? Would our guess at the proportion of the urn's beads that are red be 34% again? Maybe? 

What if we repeated this activity *many* times? Would we obtain exactly 17 red beads each time? In other words, would our guess at the proportion of the urn's beads that are red be exactly 34% every time? Surely not. 
...
What we just demonstrated in this activity is the statistical concept of **sampling**. We would like to know the proportion of the urn's beads that are red. Because the urn has a large number of beads, performing an exhaustive count of the red and white beads would be time-consuming. We thus extracted a *sample* of 50 beads using the shovel to make an *estimate*. Using this sample of 50 beads, we estimated the proportion of the *urn's* beads that are red to be 34%.
...
In the previous Section \@ref(sampling-activity), we performed a *tactile* sampling activity by hand. In other words, we used a physical urn of beads and a physical shovel. We performed this sampling activity by hand first so that we could develop a firm understanding of the root ideas behind sampling. In this section, we'll mimic this tactile sampling activity with a *virtual* sampling activity using a computer. In other words, we'll use a virtual analog to the urn of beads and a virtual analog to the shovel. 
...
urn <- tibble(color = c(rep("red", 900), rep("white", 1500))) %>%
  sample_frac() %>% 
  mutate(bead_ID = 1:2400) %>% 
  select(bead_ID, color)
urn  
```

Observe that `urn` has 2400 rows, telling us that the urn contains 2400 equally sized beads. The first variable `bead_ID` is used as an *identification variable*; none of the beads in the actual urn are marked with numbers. The second variable `color` indicates whether a particular virtual bead is red or white. View the contents of the urn in RStudio's data viewer and scroll through the contents to convince yourself that `urn` is indeed a virtual analog of the actual urn.

Now that we have a virtual analog of our urn, we now need a virtual analog of the shovel seen in Figure 6.2 to generate virtual samples of 50 beads. We're going to use the `rep_sample_n()` function included in the **infer** package. This function allows us to take `rep`eated, or `rep`licated, `samples` of size `n`. 
...
Observe in the spreadsheet viewer that the first 50 rows of `replicate` are equal to `1` while the next 50 rows of `replicate` are equal to `2`. This is telling us that the first 50 rows correspond to the first sample of 50 beads while the next 50 rows correspond to the second sample of 50 beads. This pattern continues for all `reps = 33` replicates and thus `virtual_samples` has 33 $\cdot$ 50 = 1650 rows. 
...
<!-- DK: This prints out incredibly ugly. Useful? Better way? Recall: no comments in visible R code chunks. Split up chunks in different parts, and put text between them. DONE.-->
...
<!-- DK: All tables should have a caption with their number in it. If we gave this table a sensible caption, would we need to show the code? Or perhaps the code, or maybe just the code for one element, might go in the side margin? DONE. -->
...
An actual Preceptor Table shows what we actually know. Accordingly, the table is riddled with question marks that the real world saddles us with. The ideal Preceptor Table is the Preceptor Table with no question marks, and a reasonable number of rows and columns. With an ideal Preceptor Table, we would calculate the value we want without any need for inference; the solution is a simple matter of arithmetic. 
...
urn %>%
  ...
  add_row(bead_ID = "...", color = "...", .after  = 8) %>%
  gt() %>%
  cols_label(bead_ID = md("**ID**"),
             color = "Year") %>%
  tab_style(cell_borders(sides = "right"),
            location = cells_body(columns = vars(bead_ID))) %>%
...
tibble(ID = c("1", "2", "3", "4", "5", "6", "7", "8", "...", "2399", "2400"),
       color = c("?", "?", "?", "white", "?", "?", "?", "?", "...", "red", "?")) %>%
  gt() %>%
  cols_label(ID = md("**ID**"),
             color = "Color") %>%
