...
A Preceptor Table is a table with rows and columns for all the data we would (reasonably) like to have. There are three different types of Preceptor Tables that are applicable to our urn example: Actual, Ideal, and Reduced.

The Actual Preceptor Table shows what we actually know. Accordingly, the table is riddled with question marks that the real world saddles us with. The Ideal Preceptor Table is the Preceptor Table with no question marks, and a reasonable number of rows and columns. With an Ideal Preceptor Table, we would calculate the value we want without any need for inference; the solution is a simple matter of arithmetic. Finally, the Reduced Preceptor Table...
...
Let's imagine we use our shovel and sample 100 beads from the urn. After taking our sample, we find that 40% of the *sampled* beads are red. What does this information tell us, specifically, about Bead 1? Bead 2? Remember: ball_ID is an identification variable. Let's visulize this by looking at the entire urn after our sample in a Preceptor Table.
