...
Many questions have binary (yes/no or success/failure) answers: 
...
<!-- DK: I don't think we need this Needed packages section each chapter. Or, if we have something here, we should use it as an occasion to educate readers about cool package facts. Need to improve their technical knowledge each chapter. -->
<!-- MB: Do i delete this -->
...
<!-- This chunk literally crashes my R session everytime I try to run it -->
...
3. Calculate the *empirical logit* for each group, using the logit function: $log\left(\frac{p}{1 - p}\right)$
4. Plot the results.
...
```{r, eval=FALSE}
house_south_model <- glm(dem_win ~ south, family = binomial, data = house_ch13)
house_south_model %>%
  tidy(conf.int = TRUE) %>%
...
x <- house_ch13 %>% 
  ...
  rep_sample_n(size = nrow(house_ch13), replace = TRUE, reps = 1000)

x
```

For each `replicate`, we have `r nrow(house_ch13)` resamples of `dem_win` and `south`.
...
```{r}
predict(house_logistic, new_data = house_ch13)
```
...
```{r}
house_pre_2018 <- house_ch13 %>% filter(year != 2018)
