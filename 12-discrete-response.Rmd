...
<!-- * First a regression using just data from 2018 with rstanarm with conservative ~ age, gender, race, education. (Maybe leave out education because it is an ordered factor?) Within just this example, we can go through all of our themes.Rmd issues. First do gender, then age, then multivariate. Make the model, discuss meaning of the coefficients. Then, after multivariate, you are doing posterior_linpred(), posterior_pred(). Then place the whole thing within the context of tidymodels. -->
...
<!-- MB: Using augment() for future predictions in this chapter? Works poorly with example DK: No. Only use tidymodels approach. -->

<!-- DK: table(cces$year, cces$approval) is concerning. I fixed. -->
